...
本包配合[中文正文](../2026-09-04-approval-operation-identity.zh.md)使用。[English guide](README.en.md)。性质：**公开配套的受控实验记录；不是修复回执或独立 QA。**
...
- [观察数据](observations.json)：逐轮结果及四个保留的断言失败。
- [来源定位](sources.json)：Codex PR 元数据，以及固定 CodeFlowMu 文件的 SHA-256。
- [完整性清单](manifest.json)：正文、双语说明和数据的文件摘要。
- [读取校验器](check-evidence.mjs)：运行 `node check-evidence.mjs`。
...
没有删除失败事实，也没有把 force/delete 摘要等价扩大为真实 Git 操作结果。原始研究中的工具使用错误、spec/TAP 计数修订及不完整调试轮不进入计数；相关限制在[正文第 6 节](../2026-09-04-approval-operation-identity.zh.md)及原研究记录中保留。本包只覆盖正文引用的完成轮次。
