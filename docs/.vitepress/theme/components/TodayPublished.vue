...
<template>
  <section class="today-published" :class="{ 'is-empty': !notes.length }">
    <header>
      <div>
        <small>TODAY · {{ today }}</small>
        <h2>{{ zh ? '今日发布' : "Today's releases" }}</h2>
        <p v-if="notes.length">{{ zh ? `今天已正式发布 ${notes.length} 篇研究文章，以下内容已进入公开索引。` : `${notes.length} research articles have been formally released today and are visible in the public index.` }}</p>
        <p v-else>{{ zh ? '今天的研究生产线已经开始；正式文章将在当日 Publication 完成后出现在这里。' : "Today's research production line is active. Formal articles will appear here after today's Publication completes." }}</p>
      </div>
      <strong>{{ notes.length }}</strong>
    </header>

    <div v-if="notes.length" class="today-grid">
