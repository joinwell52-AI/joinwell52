    ...
    <div class="rcv5-shell">
      ...
      <section class="rcv5-section">
        ...
        <div class="rcv5-pubs">
          ...
          <a :href="p('/en/publications/tmpa-architecture-paper-a0.9', '/zh/publications/tmpa-architecture-paper-a0.9')"><img :src="base('/assets/covers/tmpa-paper.svg')" alt="TMPA Architecture Paper"><div><span>PAPER · A0.9</span><h3>TMPA Architecture Paper</h3><p>{{ zh ? 'TMPA 理论指导、FCoP 协议与 CodeFlowMu 工程落实关系定稿。' : 'Finalized guidance from TMPA theory through FCoP protocol to CodeFlowMu engineering.' }}</p><b>Read · Cite · Versions →</b></div></a>
          <a :href="p('/en/publications/tmpa-core-specification-s0.6', '/zh/publications/tmpa-core-specification-s0.6')"><img :src="base('/assets/covers/tmpa-spec.svg')" alt="TMPA Core Specification"><div><span>SPEC · S0.6 RC</span><h3>TMPA Core Specification</h3><p>{{ zh ? 'RFC 风格发布候选版；Reference Reader 14/14 PASS。' : 'RFC-style Release Candidate; Reference Reader 14/14 PASS.' }}</p><b>Read · Cite · Versions →</b></div></a>
          <a :href="p('/en/publications/implementation-case-i0.8', '/zh/publications/implementation-case-i0.8')"><img :src="base('/assets/covers/implementation-case.svg')" alt="Implementation Case"><div><span>CASE · I0.8</span><h3>Implementation Case Report</h3><p>{{ zh ? 'CodeFlowMu V1.6.0 对照 S0.6 锁定运行：C01–C14 作者运行 14/14 PASS。' : 'Locked CodeFlowMu V1.6.0 run against S0.6: author-run C01–C14 14/14 PASS.' }}</p><b>Read · Cite · Versions →</b></div></a>
        </div>
      </section>
