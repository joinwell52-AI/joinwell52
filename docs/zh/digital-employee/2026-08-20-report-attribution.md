...
不熟悉任务归因的技术读者，可以先记住“归因三问”：

| 归因问题 | 白话解释 | 可以检查什么 |
|---|---|---|
| 谁干的 | 这份报告来自哪个项目、哪个任务、哪次执行 | 任务编号、父任务、项目根、运行系统记录的执行轮次 |
| 拿什么证明 | 报告中的成功结论有没有机器证据 | 实际命令、退出码、原始测试输出和当时的代码版本；只有真实采集过才能引用 |
| 对谁负责 | 哪份任务合同接收这组证据，谁有权验收 | 唯一任务绑定、报告替代关系、QA（独立质量验证）和最终接受决定 |

这三问分别防止三类串账：把甲的报告记给乙、把模型写出的“测试通过”当成真实日志、让执行者自己宣布验收完成。当前 CodeFlowMu 只在部分关键路径实现了下文所述硬门；“所有权威元数据都由运行系统注入”仍是目标合同，不能冒充现成功能。
...
其中“运行系统自动注入全部身份”和“统一采集全部工具证据”是目标方向；本文会明确指出哪些检查已经存在、哪些仍需实现。

## 报告存在，不等于报告属于这项任务

多 Agent（多智能体）系统常用四种简便办法归档报告：

1. 挂到同一聊天线程；
2. 挂到最近创建的任务；
3. 根据正文中出现的任务编号匹配；
4. 根据发送角色匹配，例如 DEV（开发执行者）报告挂到最近的开发任务。
...
CodeFlowMu 为一个关键 DEV（开发执行者）回执场景实现了严格的三字段检查。[`reportAttribution.ts`](https://github.com/joinwell52-AI/CodeFlowMu-open/blob/ed5634c718b9e238c44bb70851020c9793546fe6/packages/codeflowmu-runtime/src/pm/reportAttribution.ts) 会比较：

```text
REPORT 文件名推断的 TASK 编号
        ==
frontmatter.task_id（文件头中的任务编号）
        ==
references[0]
```
...
必须说明：这是 CodeFlowMu 当前某个关键 DEV 验收路径的应用级硬门，不是 FCoP（文件协作协议）对所有 REPORT（执行报告）、所有角色和所有文件名的普遍规则。FCoP 定义报告信封和引用能力；具体项目可以在宿主 Runtime（运行系统）中增加更严格的归因合同。
...
归因门不仅要说明什么可以通过，还要给出 Veto List（否决清单）。“否决”表示证据当前不足或发生冲突，不等于认定提交者恶意欺诈：

- 报告声明的代码版本与实际受测工作区不一致：判定为过期证据或版本冲突，隔离等待复核；
- 只写“测试通过”，却没有实际命令、退出码或原始输出：证据不足，不得进入成功总结；
- 文件名、任务编号和首个引用三处一致，却与运行系统的真实派工记录不符：归属失败；
- 同一任务存在多份报告，却没有执行轮次或明确的替代关系：不得自行选择“最新”文件；
- 只靠文件修改时间判断先后：不能证明因果顺序，必须结合事件序号、执行轮次或明确的替代记录；
- 若某条执行路径确实采用有时限的执行权，超过期限的报告只能进入复核；没有实现该机制的路径，不能虚构“租约过期”规则。

这份清单把“看起来像成功”转换成可执行的停止条件：系统宁可返回“归属未确定”，也不能把三份真实报告拼成一笔虚假的交付。

## 为什么需要身份传播，但不能把 Trace 当验收

分布式系统已经遇到过类似问题。W3C 的 [Trace Context（链路追踪上下文）](https://www.w3.org/TR/trace-context/)用 `trace-id`（整条调用链编号）关联一条跨服务调用链；`parent-id`（上游操作编号）是调用方为当前请求携带的上游标识，接收方创建下游请求时会用新的当前操作标识继续传播。OpenTelemetry 的[追踪 API（编程接口）](https://opentelemetry.io/docs/specs/otel/trace/api/)规定每个 span（一次调用片段）最多有一个父片段，父子操作共享 TraceId（调用链编号），从而构成因果树。
