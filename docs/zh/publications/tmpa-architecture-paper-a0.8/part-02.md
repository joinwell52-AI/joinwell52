...
| 演示 | 将 FCoP 协议及有界 CodeFlowMu、小典与 WP-13 路径映射到 TMPA 概念 | 实现可行性；RQ3 |
...
证据集合按当前架构的可追踪性和可检查工件的可用性选取，并非随机抽样。分析使用描述性计数、不变量检查、反例与工件追踪；不进行零假设检验、效应量估计或因果比较。早期 FAIL、PARTIAL 与 NOT RUN 结果继续保留在不可变的 I0.6/I0.7 历史中；A0.8 单独报告后续 I0.8 结果，不改写早期条件。
...
独立维护的 [TMPA Core Specification S0.6](/zh/publications/tmpa-core-specification-s0.6) 是当前 C01–C14 及全部 SHALL/MUST 条款的唯一规范性来源。[Implementation Case Report I0.8](/zh/publications/implementation-case-i0.8) 报告当前有边界的 S0.6 工程证据基线。I0.6 与 I0.7 继续作为各自旧输入和产品修订的不可变历史基线。本节按研究问题评估结果，不复制规范正文。
...
A0.8 是理论架构论文；Core S0.6 是当前规范性来源；Implementation Case I0.8 报告当前作者运行的 S0.6 工程证据基线。I0.6 与 I0.7 保留各自早期历史状态。论文可以总结配套工件，但不得静默重定义其含义，不得合并 Reference Reader 与产品裁决，也不得把固定 Bundle 结果扩大为普遍一致性。
...
该架构沿着**实践 → 方法 → 理论**形成：小典 AI 暴露多角色协调问题，原始 TMPA 识别文本异步方法，FCoP 抽取并成熟其可复用文件协调与复核子集，CodeFlowMu 提供下游应用反馈，本文则形式化对象、不变量、保证边界和一致性标准。早期 Pipeline 记录起源，而不是追溯性的 Core Conformance。

A0.8 在架构层回答 RQ1 与 RQ2，并为 RQ3 提供由作者运行、版本锁定的基线。在 I0.8/S0.6 评估下，CodeFlowMu V1.6.0 针对精确 Bundle 记录**产品级 14 项 PASS、0 PARTIAL、0 NOT RUN、0 FAIL**。其内置 TMPA 套件通过 23/23；Runtime 记录 1,485 passed / 0 failed / 1 skipped；Shell 记录 777/777；锁定的 FCoP 参考实现记录 1,210 passed / 2 skipped。单独维护的 S0.6 Reference Reader 通过全部 14 项合成 Fixture，但这些结果仍与产品执行分开。公开复现器提高了可检查性，却不会自动形成独立验证。
