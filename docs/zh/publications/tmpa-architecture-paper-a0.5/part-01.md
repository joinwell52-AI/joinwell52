...
本文采用设计科学方法：从治理缺口与工程谱系推导需求，构造 TMPA 人工制品，分析其不变量和威胁边界，把人工制品映射到 FCoP 参考 Profile，并通过版本锁定的 C01–C14 语料库评估 CodeFlowMu 与小典 AI 的边界证据。贡献在于一体化架构，而不是新的存储原语；评价结论严格受证据范围约束。在首个作者执行基线中，**2 项 PASS、8 项 PARTIAL、4 项未在产品 Reader 层运行**。没有直接执行的门禁标准失败，但这不构成完整一致性或独立验证。现有证据支持结构可行性；低资源性能、采用成本、比较基线、更广泛故障恢复和第三方复现仍是开放的实证要求。
...
A0.5 通过证据缺口分析回答 RQ1，通过 TMPA 的对象、流、权限、生命周期与重建模型回答 RQ2。RQ3 只得到边界明确的部分回答：参考 Profile 与案例建立部分可行性；锁定基线报告产品级 2 项 PASS、8 项 PARTIAL、4 项 NOT RUN [28]。量化安装负担、低资源性能、更广泛故障恢复、比较基线、代表性使用和第三方复现仍未完成。
