...
<ArticleCover
  ...
  version="A0.6"
  status="投稿级学术草稿"
  languageHref="/en/publications/tmpa-architecture-paper-a0.6"
...
A0.6 通过证据缺口分析回答 RQ1，通过 TMPA 的对象、流、权限、生命周期与重建模型回答 RQ2。RQ3 只得到边界明确的部分回答：参考 Profile 与案例建立部分可行性；I0.6/S0.5 基线报告产品级 1 项 PASS、8 项 PARTIAL、3 项 NOT RUN、2 项 FAIL [28]。WP-13 案例进一步说明，完成的 Agent 轨迹不会自动成为可准入的治理证据 [36]。量化安装负担、低资源性能、更广泛故障恢复、比较基线、代表性使用和第三方复现仍未完成。
...
在本文审查的相邻研究中，没有一项把以下全部性质合并为同一个最低基础设施流程契约：稳定主载体、单写者责任流、不强制总顺序的异步组合、显式权限与生命周期语义、无效和冲突证据保留，以及治理图与问题集合的确定性重建。这是 A0.6 主张的特定研究缺口。该主张是架构性与比较性的，不是对所有未公开或专有系统的优先权声明。
