...
独立维护的 [TMPA Core Specification S0.3](/zh/publications/tmpa-core-specification-s0.3) 是 C01–C14 及全部 SHALL/MUST 条款的唯一规范性来源。本架构论文只总结该契约，不重定义其含义；工程裁决属于 [Implementation Case Report I0.3](/zh/publications/implementation-case-i0.3)。
...
| ID | 规范测试名称 | 架构层可观察要求 |
|---|---|---|
| C01 | Schema 验证 | 规范对象与无效 Schema 案例得到一致处理 |
| C02 | 主载体与单写者不可变性 | 单写者发布与更正保留先前证据 |
| C03 | 重复对象 ID | 相同 ID、不同内容的候选保持冲突且不具权威性 |
| C04 | 串行流连续性与异步推进 | 局部序号故障不虚构全局总顺序，也不阻塞无关流 |
| C05 | 角色权限 | 超范围动作 Fail Closed，并产生治理问题 |
| C06 | 生命周期合法性 | 非法生命周期迁移被拒绝且不改变权威状态 |
| C07 | 职责分离 | 强制复核分离，或记录经过授权的例外 |
| C08 | 完整性篡改 | 检测被覆盖内容的篡改 |
| C09 | 缺失引用 | 必需引用缺失产生 `undetermined`、partial 或 blocked 状态 |
| C10 | 禁止环 | 检测禁止环，同时未受影响子图继续可用 |
| C11 | 聚合与重建确定性 | 相同来源集合在不同排列下产生字节等价的规范图与问题输出 |
| C12 | 冲突保留 | 互相矛盾的有效证据保持 disputed 与 `undetermined`，直至出现授权解决 |
| C13 | 恢复 | 全新 Reader 能重建责任、生命周期、依赖与问题 |
| C14 | 终态历史保留 | 终态、迁移与先前证据保持可恢复 |
