...
2026-08-10 的 S1.0 取证候选审查把中英文文档、四份机器可读 Schema、生命周期 Profile、规范化 Profile、Reference Reader 与 C01–C14 Fixture 冻结为同一个版本输入 Bundle。S1.0 在不增加新治理概念的前提下，把已经审查的 S0.6 规范设计提升到正式版本线；版本标识、机器 Schema 身份、可执行语料库路径、出版元数据与稳定版语言均按 S1.0 重新发布。

作者生成的 S1.0 Reference Reader 通过全部十四项 S1.0 Fixture。这只为已测试的 Reference Reader 路径建立 demonstrated 行为。由于尚无产品发布精确版本的 S1.0 Bundle，S1.0 产品轨道的十四项标准全部明确记为 `NOT RUN`。本次结果不建立产品一致性、独立验证、语义真实性或独立采用。

I0.8 与锁定的 CodeFlowMu V1.6.0 证据包继续作为精确 S0.6 的作者运行证据，结果为 14/14 PASS；它们 **SHALL NOT** 被改标为 S1.0 证据。本候选版用于让 CodeFlowMu 执行一次新的精确 S1.0 运行。在独立锁定的新证据包、SHA-256、输入 Bundle Digest、结果 Digest、实现 Commit 与 C01–C14 裁决正式发布前，S1.0 产品轨道继续保持 `NOT RUN`。
