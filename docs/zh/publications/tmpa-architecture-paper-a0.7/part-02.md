...
由于部分工程证据早于 C01–C14 标签，综合语料库把选定测试和工件映射到固定标准 Fixture [28]。评估分为两条轨道。**分析轨道**检查不变量、反例、等输入确定性、非法迁移、冲突与三值判断。**工程轨道**锁定版本和环境；盘点证据并计算 Hash；把工件和测试映射到每项标准；保留预期和实际输出；在不隐藏未满足前置条件的情况下分配产品裁决；并分开报告 Fixture 一致性与产品行为。WP-13 路径被视为证据准入行为，而不是 Agent 不再产生幻觉的证明 [36]。
...
证据集合按当前架构的可追踪性和可检查工件的可用性选取，并非随机抽样。分析使用描述性计数、不变量检查、反例与工件追踪；不进行零假设检验、效应量估计或因果比较。负面结果、脏工作树状态、未满足前置条件和 NOT RUN 标准均保留在基线中，以降低只选择有利结果的风险。
...
A0.7 是理论架构论文；Core S0.5 是当前规范性来源；Implementation Case I0.6 报告当前作者运行的 S0.5 工程证据基线，I0.5 保留 S0.4 历史。论文可以总结配套工件，但不得静默重定义其含义，也不得把 Reference Reader Fixture 提升为产品一致性。
...
A0.7 在架构层回答 RQ1 与 RQ2，并为 RQ3 提供由作者运行、版本锁定的基线。在 I0.6/S0.5 评估下，**产品级 1 项 PASS、8 项 PARTIAL、3 项 NOT RUN、2 项 FAIL**。FCoP v3.2.5 记录 1,222 项通过、3 项失败、2 项跳过；隔离 CodeFlowMu 快照记录 Runtime 1,420 项通过、1 项失败、1 项跳过，以及 Shell 770/770。独立 S0.5 Reference Reader 通过全部 14 项合成 Fixture，但这不替代产品执行、干净公开复现或独立验证。
