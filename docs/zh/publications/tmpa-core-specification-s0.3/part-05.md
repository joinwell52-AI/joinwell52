...
2. **FCoP Profile Conformance：** 通过有文档记录的 FCoP 投影满足 TMPA Core，并实现已发布的 FCoP 命名、生命周期、原子迁移、路由与证据规则；
...
| ID | 测试 | 规范基础 | 通过标准 |
|---|---|---|---|
| C01 | Schema 验证 | 9.2、9.3 | 缺少必填字段、Core 类型错误、禁止顶层字段、签名组不完整或声明 `date-time` 无效的对象被排除在权威集合之外，并产生确定性验证问题 |
| C02 | 主载体与单写者不可变性 | 9.2 | 一个稳定任务载体保持可识别；其他写者不能替换或共同编辑已发布对象；更正/取代通过引用或限定旧对象的新可归属证据表达 |
...
| C05 | 角色权限 | 9.4、9.9 | 超出已验证角色范围的动作保持可观察，但不应用于权威状态 |
| C06 | 生命周期合法性 | 9.6、9.9 | 未定义或未授权迁移作为问题保持可观察，且不改变权威生命周期状态 |
| C07 | 职责分离 | 9.3、9.4 | 同一身份不能执行并独立复核同一结果，除非存在 Profile 授权例外对象；例外及批准权限保留在证据中 |
| C08 | 完整性篡改 | 9.8、9.9 | 修改被覆盖内容但保留原完整性元数据会导致 Digest 验证失败；对象保留为失败证据但排除在完整权威集合之外 |
| C09 | 缺失引用 | 9.7、9.9 | 未解决必需目标出现在问题集合中，依赖不被视为已满足 |
| C10 | 禁止环 | 9.7、9.9 | 受影响的禁止环子图被隔离并报告，未受影响有效对象继续可重建 |
| C11 | 聚合与重建确定性 | 9.9 | 同一最终规范候选集合与固定 Profile 的所有测试枚举、延迟交付排列和聚合顺序，都产生字节等价的规范偏序治理图与问题集合；无关跨流对象保持不可比较 |
| C12 | 冲突保留 | 9.9 | 矛盾的有效复核保持可见且 disputed，直至出现新的授权解决对象 |
| C13 | 恢复 | 9.10 | 全新 Reader 从持久治理证据重建当前责任、生命周期、未解决依赖与问题状态，无需隐藏 Runtime 上下文 |
| C14 | 终态历史保留 | 9.2、9.6 | 进入终态或归档后，仍保留重建完整治理历史所需的旧对象与迁移 |
