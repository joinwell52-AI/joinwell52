...
| ID | 测试 | 规范基础 | 通过标准 |
|---|---|---|---|
| C01 | Schema 验证 | 4.1、9.2、9.3 | 缺少包括 `governed_work` 在内的必填字段、Core 类型或版本错误、字段被禁止、迁移元组格式错误、签名组不完整或声明 `date-time` 无效的对象被排除在权威集合之外，并确定性地产生 `SCHEMA_INVALID` |
...
| C06 | 生命周期合法性 | 6.2–6.4、9.6、9.9 | 未定义迁移产生 `ILLEGAL_TRANSITION` 与 `invalid`；当前状态有歧义或前置证据缺失产生 `LIFECYCLE_UNDETERMINED` 与 `undetermined`；两者都不改变权威状态 |
