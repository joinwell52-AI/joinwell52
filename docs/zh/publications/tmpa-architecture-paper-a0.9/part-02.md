...
| 演示 | 将 FCoP 协议及受 TMPA 指导的 CodeFlowMu、小典与 WP-13 路径追踪到 TMPA 概念 | 实现可行性；RQ3 |
...
FCoP 不等同于 TMPA。它通过命名文本工件、生命周期目录、面向追加的迁移证据、Schema、Runtime 工具与适配器，实现 TMPA 的文件型子集 [7]–[12]。CodeFlowMu 是在 TMPA 指导下开发、并以 FCoP 作为协作协议的工程系统，承载持久多角色开发与工作环境 [13]、[37]。
...
历史谱系为小典 AI → 早期 TMPA → FCoP → CodeFlowMu，随后工程反馈进入当前 TMPA 形式化。小典暴露了跨模型 Session 保留任务接受、执行、复核、拒绝与审计证据的需求 [25]；早期 TMPA 提供文本异步方法；FCoP 抽取可复用的协调与复核骨架；CodeFlowMu 则把这一方向落实为更大规模的持久工作系统。其运行结果继续提供证据和修订输入，但不会成为定义 TMPA 的权威来源。
