...
TMPA 的 **SME-first** 不是“只能服务于中小企业”，而是一条更强的架构约束：**治理语义不应被内生锁死在某个模型会话、某个进程、某个中央状态机，或某一种物理存储拓扑中。**
...
每一个动作都拥有自己的作者和引用关系。谁写下 `Acceptance`，谁追加 `Report`，谁发布 `Correction`，都会沿着作者、引用与局部顺序进入同一条可追溯的事实链。
...
不同的单写者串行流可以独立推进。每个写者只负责维护自己的局部顺序，不需要抢占一个全局可变状态来证明“谁先谁后”。
