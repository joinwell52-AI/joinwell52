...
静态入队检查也不够：如果运行中的 Agent 可以新增或改写下游依赖，那么每一次依赖变更都必须重新接受环路校验，或被显式禁止。超时只能报告“等待超过策略窗口”，不能猜测该删哪条边或自动宣布业务失败。当前材料没有证明动态依赖变更、死锁超时或破环裁决协议。未来版本的派工器测试必须把“包含环的依赖图”列为明确的准入拒绝用例：无论提交时形成环、执行中引入环，还是长期没有上游进展，都应产生可检查的问题记录并交给有权主体，而不是无限等待或启发式选边。
