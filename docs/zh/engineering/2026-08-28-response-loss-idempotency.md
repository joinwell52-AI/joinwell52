...
这不是抽象推演。CodeFlowMu 在一次故障注入里得到了一组非常清楚的对照：同一个响应丢失窗口下，`write_report` 最终仍只有一份报告，而 `write_task` 却产生了两张 TASK。

这组正反结果把问题从“系统要不要加去重”收窄成一个更具体的工程问题：

> 第二次调用到来时，创建接口能不能从持久事实中证明：这是不是第一次业务提交的恢复，而不是一个新的创建意图？

这篇文章记录的不是一个孤立 Bug，而是 CodeFlowMu 从“能执行工具”走向“能可靠承担数字员工运行任务”时补上的一条 Runtime 边界：**持久化幂等边界。**
...
外部案例只负责提出问题。CodeFlowMu 是否存在同类工程缺口，必须由自己的实现和第一方实验来回答。

## 第一方复现：同一故障窗口，两条写路径给出相反结果
...
恰恰相反，`write_report` 是一个重要反例。它说明上层重试并不必然制造重复副作用，只要底层接口已经拥有可恢复的持久身份与结果复用机制。

真正的缺口集中在任务创建：当时 `write_report` 已经暴露 `client_submission_id`，而 `write_task` 还没有相应的正式持久幂等合同。调用方主观上认为“这还是刚才那一次提交”，并不等于 Runtime 已经持久保存并认可了这个业务身份。

## 工程判断：可靠重试不是“去重”，而是恢复第一次结果

发现两张 TASK 之后，最容易想到的修法是：创建前查一下有没有同标题任务，有就不创建。

这不是可靠幂等。

标题、正文摘要、发送者甚至时间窗口都可能相同，却代表两次真正独立的业务意图。反过来，同一次业务提交在恢复时也可能带着不同的 trace、连接、进程或重试次数。如果用业务内容做模糊匹配，Runtime 既可能漏掉重复，也可能把合法的新任务误判为旧任务。

因此，CodeFlowMu V2.1.2 没有把问题定义成“怎么更聪明地查重”，而是定义成：

**如何给一次创建意图一个稳定、可持久化、可恢复、可冲突检测的提交身份。**

这也是这次工程化最关键的变化：重试从调用方行为，变成了创建接口合同的一部分。
...
其中 `client_submission_id` 描述的是一次业务创建意图，而不是某一次 HTTP、MCP 或进程调用。需要跨响应丢失、跨进程恢复的调用方，必须在重试时复用同一个稳定 ID。
...
V2.1.2 的创建回执不再只返回“成功/失败”，而是明确区分：

| `disposition` | `action_taken` | 含义 |
| --- | --- | --- |
| `created` | `true` | 本次调用完成了首次创建 |
| `reused` | `false` | 对应创建结果已经存在，返回第一次的权威任务身份 |
| `conflict` | `false` | 同一提交身份与已有摘要或恢复事实冲突，没有新增 TASK |

这里一个很容易被误读的字段是 `action_taken=false`。

在 `reused` 场景里，它不是“请求失败”，而是恰恰证明 Runtime 没有再执行第二次创建，却仍然成功找回了第一次结果。

需要继续限定的是：这里的“成功”只表示任务对象创建结果被恢复，不代表任务已经执行完成、QA 已通过或业务已经交付。技术幂等不能替代上层治理状态。

兼容路径也没有被包装成更强保证。旧调用方不提供 `client_submission_id` 时，仍然可以使用原有 legacy 创建方式；但这种调用不会被宣称拥有跨响应丢失、跨进程的强幂等语义。

## 独立 QA：不是看“有没有报错”，而是看最终只剩几个权威对象

实现完成之后，未参与实现的 QA 在候选版本上独立执行响应丢失和并发场景。验收关注的不是一句“测试 PASS”，而是实际 TASK 数量、`task_id` 是否一致，以及创建回执是什么。
...
V2.1.2 同时覆盖进程重启后的结果复用、同 ID 异摘要冲突、legacy 返回兼容、中间态文件缺失与 stale reservation 诊断等定向测试。

这里也必须保留证据边界：A2/A4 的独立 QA 证明的是受测任务创建合同在这两个关键故障模型中收敛，不等于“所有工具、所有 Host、所有网络环境都已经被独立验证”。

但对这次工程问题而言，前后差异已经足够明确：

**历史受测版本中，同一创建意图在响应丢失后可以产生两张 TASK；V2.1.2 中，提供稳定提交 ID 和一致摘要的受测重试、并发与重启路径会收敛到同一个权威任务身份。**

## 从补丁到产品版本：还必须经过发布门禁

V2.1.2 于 2026-08-30 正式发布，发布类型是母版 Runtime 边界安全补丁。
...
这些数字是发布测试集合的实际结果，不应该被相加成一个所谓“幂等可靠率”。

本次发布对象是 CodeFlowMu 私有母版 Runtime/Shell，不是 Open Dev Team Edition，也不代表真实 LAN/Gateway、浏览器 profile 或用户生产项目已经被同一套夹具覆盖。Windows 符号链接权限导致的 1 项 skip、既有依赖审计告警以及 Python 定向夹具 warning 继续保留在发布证据中。

[CodeFlowMu V2.1.2 发布说明](https://github.com/joinwell52-AI/codeflowmu/blob/main/docs/releases/V2.1.2-RELEASE-NOTES.md)

## 这次工程化真正改变了什么

从产品视角看，V2.1.2 并不是简单地“给 `write_task` 加了一个去重参数”。它把任务创建的失败模型重新定义了一遍：
...
这使“重试”从一种带猜测的调用策略，变成了一个可以被持久记录、恢复、审计和测试的工程合同。
...
- [历史实验及 V2.1.2 工程更新说明](/zh/research/evidence/2026-08-28-response-loss-idempotency)：公开 JSON fixture、Reader 与 check 用于验证冻结历史材料的一致性，不运行私有 Runtime，也不能拿历史 TASK=2 的 fixture 判断 V2.1.2 当前行为。
- [LlamaIndex PR #22841](https://github.com/run-llama/llama_index/pull/22841)：提供外部故障模型与调用形式修复背景，不作为 CodeFlowMu 的实现或验收证据。
- [CodeFlowMu V2.1.2 发布说明](https://github.com/joinwell52-AI/codeflowmu/blob/main/docs/releases/V2.1.2-RELEASE-NOTES.md)：记录持久幂等边界、兼容性、独立 QA、R3 发布门禁与残余风险。
- V2.1.2 实现、独立 QA 与发布原始日志属于受限第一方材料；公开页只给出来源编号与脱敏结果。真实生产发生频率、全部操作系统、真实 LAN/Gateway 与用户生产项目不在本文证据覆盖范围内。
