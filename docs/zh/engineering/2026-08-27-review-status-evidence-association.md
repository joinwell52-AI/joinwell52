...
summary: "R2 最初只是从 10 条历史 REPORT 中发现‘生命周期位置不能证明证据归属’。随后这个原则进入 CodeFlowMu，经历真实任务误报纠偏，并在 V2.0.4 中成为会随 active→review 动态重算、且能正确区分已关联与当前阶段不适用的证据关联诊断。"
...
<ArticleCover
  ...
  kicker="开源工程观察 · 工程研究"
  title="从‘证据不能串账’到动态诊断：CodeFlowMu V2.0.4 如何把研究结论做成工程能力"
  summary="这不是一项先有产品、再补解释的功能。它从历史证据中的关联断点开始，经过真实任务反例纠偏，最后变成会随生命周期动态重算、且不替交付与验收签字的只读诊断能力。"
...
如果研究停在这里，它只是一条设计原则。真正有价值的部分发生在后面：我们把这条原则做进 CodeFlowMu；第一版诊断在真实任务上出现误报；我们再用现场事实修正诊断语义；到 V2.0.4，又用另一张真实任务做正向验证——同一任务从 `active` 进入 `review`，诊断链随阶段准确变化，没有把“不适用”误报成“缺失/冲突”。

这是一条完整的 **研究发现 → 工程实现 → 真实反例 → 语义修复 → 正向验证** 路径。
...
## 4. V2.0.4 修复后的正向验证：同一任务 `active → review`，状态变化准确且没有误报

这里必须和上一节严格区分：**下面两张截图不是误报现场。它们是 V2.0.4 修复完成后的正向验证样本。**

两张截图对应同一张任务：

`TASK-20260827-030-PM-to-QA`

第一张截图时，它处于 `active`；第二张截图时，同一任务已经进入 `review`。

这组前后对照清楚证明了一件事：**诊断不是静态展示，而是会根据正式生命周期和当前证据重新计算；在这组可见关系上，阶段变化后的诊断结果与实际流程一致，没有出现假缺失或假冲突。**
...
### 阶段 A：`active` —— “当前阶段不适用”被正确识别，没有误报缺失
...
而 REPORT 相关两条链并没有被误报成缺失或冲突：

- `REPORT → Task`：**当前阶段不适用**，`lifecycle_does_not_require_report`
- `REPORT → REVIEW`：**当前阶段不适用**，`report_not_available`

这里最关键的事实就是：**`missing=0`、`conflict=0`。**

因为任务还在执行阶段，正式 REPORT 尚未成为当前阶段要求。诊断器没有为了“检查完整”而制造一个缺失问题，而是准确标记为“不适用”。

### 阶段 B：`review` —— 随生命周期变化，REPORT 链路准确转为已关联

同一张任务随后进入 `review`。第二张截图中，诊断随着正式事实变化重新计算：
...
而当前没有产生的 `EVAL → REVIEW` 仍然准确显示：

**当前阶段不适用 · `eval_not_present`**

这组变化的意义非常直接：在 `active` 时不该要求的 REPORT 关系没有被误报；进入 `review`、REPORT 和 REVIEW 真正出现后，对应两条边自动变成已关联；没有 EVAL 时，系统也没有凭空补出 EVAL 关系。

换句话说，这不是“页面状态跟着改了颜色”，而是：

> **同一任务的证据要求随生命周期变化，诊断结果也随正式事实变化，并保持与阶段语义一致。**
...
这就是“动态诊断”与“静态展示”的区别，也是这两张截图最重要的一手价值。
...
**第五步：用另一张真实任务做正向验证。** `TASK-20260827-030-PM-to-QA` 从 `active` 进入 `review`：执行阶段 REPORT 关系正确保持“不适用”，进入审查后 REPORT → Task / REVIEW 自动变成已关联，且可见链路没有出现误报。

这才是这项工作的研究价值：**理论不是一句架构口号，工程也不是理论的简单翻译。二者之间必须允许真实现场反过来修改我们的判断，再用新的现场验证修复后的判断是否准确。**
...
## 结论：最好的工程能力，往往不是把理论“实现出来”，而是让理论经得起实现后的反驳和再次验证
...
而同一任务从 `active` 到 `review` 的两张截图，给出了修复后的正向证据：**状态变化准确，没有误报；该出现的关系在该出现时出现，不该要求的关系则保持“不适用”。**

> **证据诊断不是给任务贴一个静态标签，而是随着正式事实变化，持续准确回答“现在有哪些关系能够被证明”。**
...
- `TASK-20260827-030-PM-to-QA` 的 active / review 对照来自两张同任务本地实机截图，是**修复后的正向验证样本**：本文只在截图实际展示的关系范围内描述“阶段变化准确、未见误报”，不外推为所有任务和所有路径都已认证。
