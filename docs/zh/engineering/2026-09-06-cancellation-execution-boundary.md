...
| 场景 | 实际结果，两轮一致 | 能得出的判断 |
| --- | --- | --- |
| 尚未批准，先取消，再执行 | 取消被接受；执行被拒绝；文件效果 0 次 | 待审批取消保护有效 |
...
更关键的实验，是在审批服务已经进入执行器回调后，放置一个可控等待点。回调可以理解为审批服务交给具体执行代码的入口。我们先让它停在屏障上，发出取消请求，最后释放屏障。
...
第二组是有意设置的机制对照：研究回调在等待结束后看到中止标记，于是没有调用真实文件执行器。它证明“检查放在哪里”会影响结果，却没有证明产品已经有这项检查，更没有证明一次有效授权被成功撤销。
...
本轮没有证明 CodeFlowMu 存在“成功取消后仍启动工具”的缺陷。它证明的是，已有保护、接口职责和未验证窗口必须先分清，才知道下一步该测什么，以及究竟需不需要开发。

**可靠的取消，不是让所有地方都写着“已取消”，而是能说明：请求在哪一层被接受，阻止了哪一步，还有什么事实没有被改变。**

## 证据与范围

本篇使用 7 个审批场景和 3 个适配器场景，各两轮；不是生产事故统计，也不是安全准确率。研究还对两个既有测试文件分别运行两轮，每轮 39 项通过、0 失败、0 跳过，不能把它们相加成端到端保证。
