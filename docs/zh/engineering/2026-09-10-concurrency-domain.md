...
research_question: "并发上限设为 1，为什么两个任务仍能同时运行？"
summary: "AG2 的原方法对照实验揭示两件不同的事：循环内超限可以被修复，而整个 Agent 的总额度仍需要另一份明确合同。"
...
<ArticleCover image="/assets/authority-scope-20260910/02-concurrency-domain.cover-v1.png" kicker="开源工程观察 · 实验研究" title="并发上限设为 1，为什么两个任务仍能同时运行？" summary="AG2 的原方法对照实验揭示两件不同的事：循环内超限可以被修复，而整个 Agent 的总额度仍需要另一份明确合同。" version="2026-09-10" languageHref="/en/engineering/2026-09-10-concurrency-domain" languageLabel="English" />
...
AG2 是一个构建和编排 Agent 的开源框架。[PR #3243](https://github.com/ag2ai/ag2/pull/3243)修复子任务准入时的信号量缓存。信号量可以理解为可借出的有限名额：名额用完，后来的协程等待；任务结束，名额归还。
...
1. 循环 A 的第一个任务 A1 进入，占住一个名额。
2. 另一个真实线程中的循环 B 启动 B1。
3. A1 和 B1 都未结束，循环 A 再申请 A2。
4. 记录各循环和整体在途峰值，最后统一释放。
...
![同一实验的局部峰值与全局峰值](/assets/authority-scope-20260910/02-concurrency-domain.figure.zh.png)
...
第一是**额度所属范围**：单循环、单 Agent、单进程、单宿主，还是整个租户？第二是**谁有权发放额度**：一份内存对象，还是所有执行者共享的计数服务？第三是**额度持续多久**：任务返回、进程退出、租约到期或人工清理，分别意味着什么？第四是**发生不确定性时怎么处理**：旧任务失联，不应仅因观察不到就默认归还它可能仍在占用的名额。
...
[English](/en/engineering/2026-09-10-concurrency-domain) · [实验方法、固定版本与逐轮结果](/zh/research/evidence/2026-09-10-authority-scope)
