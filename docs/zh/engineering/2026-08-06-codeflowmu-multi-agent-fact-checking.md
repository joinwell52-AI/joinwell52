...
## 彩蛋：这篇文章发布后，Claude 又复现了一次同类错误

这篇文章发布后，我把链接发给 Claude，让它继续审稿。我按它的修改意见更新并重新发布后，Claude 却连续判断“页面没有更新”。它反复调用同一个 `web_fetch` 路径，看到旧内容，就把结果进一步解释成“站点或 CDN 仍在返回旧版本”，并坚持每次调用都是一次新的网络请求。

问题不在于工具没有返回内容，而在于 Claude 把“工具调用完成”升级成了“返回内容就是最新事实”，又把同一个工具、同一个信息源的重复调用，当成了多次独立验证。

我随后在两台电脑上打开同一页面，看到的都是新版，并把截图发给它。到这一步，Claude 才承认：

> **我把“调用完成”直接当成了“结果就是最新事实”。**

### 四张现场截图：同一工具自证，直到独立观察介入

**截图 1：Claude 坚持每次 `web_fetch` 都是“全新的、独立的网络请求”，并把六次相同结果解释为站点服务端或 CDN 返回旧版本。**

![Claude 彩蛋截图 1：把重复抓取解释为独立网络请求](../../public/assets/covers/wp13-claude-easter-01.png)

**截图 2：在看到新版页面截图后，Claude 承认新版真实存在，但仍继续用“CDN 边缘节点不同步”解释自己的旧结果。**

![Claude 彩蛋截图 2：承认新版存在后仍推断 CDN 边缘差异](../../public/assets/covers/wp13-claude-easter-02.png)

**截图 3：Claude 开始承认方法错误——它反复使用同一个工具、同一个信息源自证，没有像 PM 或 QA 那样主动切换到独立来源交叉核验。**

![Claude 彩蛋截图 3：承认没有引入独立信息源](../../public/assets/covers/wp13-claude-easter-03.png)

**截图 4：在两台电脑都显示新版后，Claude 最终明确承认，它把“工具调用完成”直接当成了“结果就是最新事实”。**

![Claude 彩蛋截图 4：承认把调用完成误当成最新事实](../../public/assets/covers/wp13-claude-easter-04.png)
...
这与 WP-13 的主案例是同一种治理问题。正文里，DEV 的 `completed` 不能覆盖磁盘、Git、REPORT 和测试事实；这个彩蛋里，Claude 的多次 `web_fetch` 也不能覆盖两台设备实际看到的新版页面。领域不同，错误结构相同：**一个 Agent 可以提出结论，但这个结论不能靠同一条工具链反复自证，就自动晋升为事实。**

两台电脑的交叉确认不是外部第三方审计，但它至少引入了独立于 Claude 单一抓取路径的观察，足以迫使原结论重新接受核验。

这个彩蛋也保留自己的证据边界：它不证明旧内容的根因一定是 Claude 本地缓存、CDN 或某个边缘节点；它只证明，在根因尚未确定时，Claude 已经把单一抓取路径返回的旧内容升级成了确定结论，并继续用同一来源为自己背书。

> **最危险的不是工具返回了旧内容，而是 Agent 在没有独立证据时，对旧内容作出了过度确定的解释。**
