...
如果答案是“是”，可观测性很快就会变成数据透传。

一个新字段只要进入内部 `payload`，就可能顺着 Activity API、Web Panel、Analytics 或日志中心一路扩散。今天开发者记得删 `payload.raw`，明天新增一个嵌套 `original`、`debug_context` 或未知事件类型，旧删除逻辑未必知道它存在。
...
V2.1.2 因而没有选择“再删几个敏感字段”，而是把内部事件与消费者对象分开：普通消费路径用服务端递归白名单重新构造安全投影，`payload.raw`、未知字段和未知嵌套值默认不进入结果；真正需要原始事件的诊断路径继续保留在明确授权的内部边界。

这篇文章记录的，是 CodeFlowMu 从“事件能被记录和查询”走向“事件按消费者职责可见”的工程化过程。

完整历史数据、探针与版本证据见：[RBE-20260828-02 证据页](/zh/research/evidence/2026-08-28-event-consumer-visibility)。
...
真正危险的是第三层没有独立合同，导致“内部可见”被误认为“普通消费者可见”。
...
问题是黑名单只认识今天知道的字段。明天出现：

- `payload.debug.original`；
- 新 Host 添加的 `native_payload`；
- 一个从未见过的新事件类型；
- 某个对象内部再嵌套一层 raw 数据；
...
这正是“内部留证”和“外部/普通消费最小化”可以同时成立的原因。
...
未知事件、新增嵌套字段、消费者对象隔离、服务端消费者选择和 Analytics 持久化等还有开发定向测试。
...
如果所有消费者都读取内部完整对象，任何 Host、工具或 Agent 新增字段都可能意外改变：
...
V2.1.2 于 2026-08-30 正式发布。Activity 安全投影与任务持久化幂等、Session 身份核验一起构成本次 Runtime 边界安全补丁。
...
本次没有发布独立 Open Dev Team Edition，也没有把真实浏览器 profile、LAN/Gateway 或用户生产项目伪装成已经被夹具覆盖。Windows 符号链接权限导致的 skip 和既有依赖告警继续保留在发布证据里。
...
7. Analytics、日志中心、Panel 和 API 是否使用一致的安全投影原则，还是各自形成泄露旁路？
...
- [历史数据、旧查询探针与 V2.1.2 更新说明](/zh/research/evidence/2026-08-28-event-consumer-visibility)：旧 JSON fixture、Reader 与 check 验证冻结历史材料，不运行当前 Runtime；历史 `current_query_returned_raw_marker=true` 不能用于描述 V2.1.2 当前行为。
...
- [CodeFlowMu V2.1.2 发布说明](https://github.com/joinwell52-AI/codeflowmu/blob/main/docs/releases/V2.1.2-RELEASE-NOTES.md)：记录递归白名单、安全投影、B1 独立 QA、兼容性、发布门禁与残余风险。
- 内部完整数据存在不等于发生过未授权访问；实现、独立 QA 与发布原始记录属于受限第一方材料。本文不声称整个 Runtime 已经消除所有信息风险，也不把真实网络部署、未来未知消费者或独立 Open Edition 描述成已经完成本次验收。
