...
research_question: "实际工具入口是否保留了批准必须绑定的动作区别，并将会话上下文与动作身份正确分开？"
...
<ArticleCover
  ...
  summary="在同一条 Agent 审批链上，改 Git 分支没有改变授权摘要，只换 Session 却改变了摘要。真实服务、跨进程记录和正反对照揭示：风险分类、动作身份与执行授权，不能交给同一个含糊的‘相同’。"
  version="2026-09-04"
  languageHref="/en/engineering/2026-09-04-approval-operation-identity"
...
最值得追查的是两个反向结果：改分支被接受，只换会话却失配。完整探针保留了这两类预期偏离，每类两次，最终以断言失败退出。没有把它改成一个漂亮的全绿报告。逐轮记录见[配套证据说明](/zh/research/evidence/2026-09-04-approval-operation-identity)。
...
[![图 1：不同分支命令在提取后生成相同批准对象](https://joinwell52-ai.github.io/joinwell52/assets/approval-identity-20260904/assets/operation-collapse.zh.png)](https://joinwell52-ai.github.io/joinwell52/assets/approval-identity-20260904/assets/operation-collapse.zh.png)
...
[![图 2：顶层 Session 被排除，嵌套 Session 仍进入摘要](https://joinwell52-ai.github.io/joinwell52/assets/approval-identity-20260904/assets/nested-session.zh.png)](https://joinwell52-ai.github.io/joinwell52/assets/approval-identity-20260904/assets/nested-session.zh.png)
...
当前状态是**研究完成、建议进入工程评审**。本文不宣布修复、合同冻结或开发授权。配套[证据包](/zh/research/evidence/2026-09-04-approval-operation-identity)提供去标识逐轮观察、主张映射与读取校验器；校验器检查记录一致性，不冒充重新运行产品或独立 QA。完整原始夹具及产品源码复跑环境仍是本地研究材料，未随此稿发布；不能把附包称为读者可直接执行的完整产品复现环境。
