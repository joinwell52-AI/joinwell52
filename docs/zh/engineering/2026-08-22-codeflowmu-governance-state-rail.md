...
> [!NOTE]
> **系列阅读顺序（1/3）**
> 本文解释三层责任；[第二篇](./2026-08-22-agent-task-file-state-machine)检查 TASK 文件状态机、原子迁移与并发边界；[第三篇](../digital-employee/2026-08-22-agent-rail-decision-boundary)讨论轨道机可以机械处理什么、必须把什么留给 Agent、PM 或 ADMIN。三篇共享同一组术语与证据边界，不能把任何一篇单独读成完整实现承诺。
...
这里有一个容易混淆的并发边界。REPORT 与 REVIEW 是独立工件，分别写入自己的信封，因而不应共同覆盖同一份报告；但两个调用者争抢同一 TASK 的生命周期迁移，仍然是同一个竞争问题。FCoP 的规则不是“谁写完 REPORT 就自动搬动任务”，而是只有 L1 生命周期工具才能改 `_lifecycle/` 拓扑：`claim_task` 把 `inbox` 移到 `active`，`submit_task` 才把 `active` 移到 `review`。标准模式在同一文件系统边界内把追加事件后的内容写入目标目录临时文件、持久化，再以 `rename` 发布目标文件，随后才清理源文件；这使目标发布具有原子可见的提交点，却不等于存在全局锁，也不自动解决两个并发领取者或跨主机写者。

因此，`rename` 不能裸奔地承担多领取者抢单。若未来支持多个独立 Runtime 争抢同一 TASK，领取入口必须有独立的、不可覆盖的互斥保留或原子比较—交换；单独在 `rename` 前检查 `expected_revision` 仍会留下检查—执行窗口，不能作为唯一认领保证。具体原语必须按目标 Host 和文件系统验证。当前本地优先的单 Runtime 部署并未声明支持这种共享工作区竞争，也没有相应压力测试；这里是未来扩展的准入要求，不是对 V1.9.7 已有机制的宣称。
...
当前合同并没有把 `unknown_reconcile` 写成一套固定的自动回滚、自动熔断、隔离队列或通知流程；这些属于调用方和运行时策略，不能从这个枚举本身推导出来。高风险下游如果不能在来源冲突时继续，合同必须显式把“对账完成”写成下游前置依赖，使下游返回 `waiting_dependency`；若当前动作应被硬拒绝，则必须命中预先冻结的确定性条件。不能仅凭枚举名把未知状态自动升级为“隔离”或“失败”。

三篇中的“双阶段路径冲突”“来源无法归一”和“身份字段不足”都指向这一个 `unknown_reconcile` 处置，不是三套平行机制。但枚举统一还不等于运行机制完整。一个可无人值守运行的对账信封至少还要绑定 `reconcile_owner`、触发原因、涉及的规范任务身份与修订、`opened_at`、处理截止时间、升级路径和允许的终止结果；处于该状态的任务不得继续消耗模型 Token 空转。V1.9.7 当前证据没有证明这些字段、SLA、通知和熔断已经形成统一实现，因此本文把它们列为后续准入条件，而不是现有能力。

同一版本还使用精确的角色工具清单检查 Agent 能否调用某项工具，并让 PM 的任务命令经过统一命令内核，核对任务身份、作用域、当前修订和防重复流水号。这些机制让“谁在什么任务里做什么”进入确定性软件边界，而不是只依赖提示词自律。

这里还要补上一条跨层契约：**依赖图是治理承载物，不是普通执行数据。**执行 Agent 可以报告“现有依赖不成立”并提出改边建议，但不能因为自己正在运行就直接改写已存在 TASK 的依赖。依赖创建或修改必须由 PM、ADMIN，或被当前治理策略明确委派的主体发起；变更要生成新的任务修订，重新经过身份、作用域和环路校验，并留下谁在什么策略版本下改变了哪条边的证据。当前材料没有证明所有动态依赖写入路径都已经统一执行这项合同，因此系列不把它写成 V1.9.7 的已验证功能。

### 治理规则怎样进入工程实现

TMPA 的治理判断不能只停留在论文里，也不能散落成 CodeFlowMu 中无法追踪的 `if`。合理的工程边界是一份**版本化治理策略包**：它明确策略 ID、版本、内容摘要、适用 Runtime 范围、角色与作用域谓词、身份规范化规则、依赖变更权限、机械拒绝清单、迁移要求和回滚条件。每次轨道处置都应记录实际消费的策略版本。

“可更新”不等于运行中静默热替换。同一任务轮次应绑定稳定策略；新策略经过校验和 ADMIN 授权后，只能在声明的生效边界切换。并发安全补丁也不能绕过这条规则：独占保留或比较—交换只解决“不能出现两个赢家”，而抢锁失败应映射为等待、拒绝还是对账，属于治理语义。紧急补丁可以先关闭多领取者能力或收紧到单 Runtime；若要改变谁能取得执行权或新增机械拒绝条件，必须先升级策略包和证据矩阵，再升级实现。

需要强调：角色工具门禁只检查规范工具身份与活动能力，它自己不解析所有命令副作用，也不等于操作系统沙箱。它只能作为协作合同的合规过滤器，不能替代按 Host 采用的操作系统级隔离，例如受限账户、文件权限、容器或平台原生的进程隔离。若 Agent 已获得可直接越过应用层的宿主权限，角色门禁本身不能阻止它修改越权文件或启动越权命令；把治理合同当安全沙箱是架构误判。当前材料没有证明 CodeFlowMu 已统一部署这些隔离措施。当前母版源码是私有的；本文只公开足以解释合同的短摘录，不把私有代码写成可公开复现的第三方证据。

## 一项任务怎样穿过三层

用“为现有系统增加 CSV 导出”作为贯穿示例，可以看到三层并不是三套平行文档。

| 阶段 | 输入 | 责任主体 | 输出 | 等待条件 | 硬拒绝条件 | 对账 / 不确定 |
|---|---|---|---|---|---|---|
| 建立工作 | 已批准的需求与验收条件 | PM / ADMIN | 稳定根任务与责任边界 | — | 身份或授权范围不成立 | — |
| 创建任务 | 任务接收者、依赖、交付物 | FCoP L1 `create_task` | 位于 `inbox` 的 TASK 文件 | — | 信封或路径不合规 | — |
| 领取并绑定执行 | `inbox` 中的 TASK、身份与角色能力 | 获得权限的 Agent 显式调用 FCoP L1 `claim_task`；CodeFlowMu 在外围建立或调度任务上下文 | TASK 迁入 `active`，并附带 `inbox → active` 事件；任务绑定到 Agent 会话 | 明确依赖未完成 → `waiting_dependency` | 重复执行或角色能力缺失 → 拒绝当前领取 | 迁移冲突 → 保留并对账 |
...
| 提交任务进入审查 | `active` TASK 与已经写入的执行回执 | 执行者显式调用 FCoP L1 `submit_task` | TASK 迁入 `review`，并追加 `active → review` 事件 | — | 明确的提交前置条件不成立 | 迁移冲突 → 保留并对账 |
| 重建事实 | TASK、REPORT、生命周期和来源 | 治理 Reader / 事实内核 | 可检查的任务快照与问题集合 | — | — | 来源缺失或冲突 → `unknown_reconcile` |
...
## 从单 Runtime 到并发生产还差什么

“当前不支持”不能成为永久回避并发问题的句号。更诚实的路线应按一致性边界分阶段：

| 阶段 | 目标拓扑 | 必须新增的准入证据 |
|---|---|---|
| 当前 | 单机、单项目根、单 Runtime Writer | 重启恢复、项目根绑定、单写者锁与现有回归 |
| 下一阶段 | 同机多进程竞争同一任务 | 不可覆盖认领、失败方零副作用、租约/回收、并发压力与故障注入 |
| 再下一阶段 | 多 Runtime 共享同一本地文件系统 | 规范身份的唯一领取、策略版本一致、双写冲突保留与可重复恢复 |
| 多主机 | 网络文件系统或跨信任域 | 不能直接沿用本地 `rename` 结论；需要单独的一致性 Profile、故障模型和权限模型 |

因此，CodeFlowMu 的数字员工产品路线确实会遇到多实例生产问题；但跨过这条线需要新的协议 Profile、策略版本和测试证据，不是把“本地优先”四个字删除就算完成。
