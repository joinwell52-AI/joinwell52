...
research_question: "相同任务在同一协作系统中，为什么产生不同的派单、交付和失败结果？"
summary: "用CodeFlowMu与FCoP实测六个AI团队：比较PM派单、交付质量、效率与故障恢复，并用独立EVAL和文件证据核查结论。"
description: "用CodeFlowMu与FCoP实测六个AI团队：比较PM派单、交付质量、效率与故障恢复，并用独立EVAL和文件证据核查结论。"
...
head:
  ...
  - [meta, { property: "og:title", content: "6个AI，同一道题，会怎么样？" }]
  - [meta, { property: "og:description", content: "用CodeFlowMu与FCoP实测六个AI团队：派单、交付、评分、独立EVAL与失败证据。" }]
  - [meta, { name: "twitter:card", content: "summary_large_image" }]
  - [link, { rel: canonical, href: "https://joinwell52-ai.github.io/joinwell52/zh/research/2026-09-09-codeflowmu-six-ai" }]
  - [meta, { property: "og:url", content: "https://joinwell52-ai.github.io/joinwell52/zh/research/2026-09-09-codeflowmu-six-ai" }]
...
<ArticleCover
  ...
  kicker="研究文章 · 六AI团队协作实测"
  title="6个AI，同一道题，会怎么样？"
  summary="用CodeFlowMu与FCoP实测六个AI团队：比较PM派单、交付质量、效率与故障恢复，并用独立EVAL和文件证据核查结论。"
  version="2026-09-09 实测"
  status="文件证据 · 独立EVAL · 六维评分"
  languageHref="/en/research/2026-09-09-codeflowmu-six-ai"
...
<div class="cfm-six-ai-stats" role="group" aria-label="测试结果概览">
  <div><strong>6 个模型</strong><span>同一任务正文 · 多角色协作</span></div>
...
## 一、CodeFlowMu与FCoP：产品、任务和测试方法 {#chapter-1}

### 1.1 CodeFlowMu与FCoP分别是什么 {#section-1-1}
...
EVAL有三条不同报告路径：**任务记录报告**分析一轮完整执行，**系统观察报告**分析系统资产，**关单旁观报告**核对PM终报与任务证据链。它们都包含分析，但触发方式、观察范围与技能组合不同，详见[1.10 EVAL报告与技能矩阵](#eval-report-matrix)。本文仍对照原始文件复核EVAL结论，并保留未生成、失败、重试和材料错配；统一评估配置不意味着每轮成功产出全部报告，`auto-smart`也不能证明底层模型始终固定。

[![CodeFlowMu任务页：豆包轮次的根任务、三角色子任务和实时过程](/articles/codeflowmu-six-ai-20260909/assets/scene-task-panel.png)](/articles/codeflowmu-six-ai-20260909/assets/scene-task-panel.png)

点击截图查看原始像素，避免缩略图影响阅读。
...
### 1.2 测试任务原文：六个AI收到同一道题 {#section-1-2}
...
页面中的任务标题用来区分被测模型，例如“系统检查codex”“系统检查豆包”“系统检查Qwen”；共同测试题是上面这段正文。原文中的“轨机”保留原样，指向本次检查的运行系统。
...
### 1.3 测试仪器与任务 {#section-1-3}

用 CodeFlowMu，实测六个 AI 的团队工作能力。仪器保存过程，并不替代 PM 作出业务判断。

[![测试结构](/articles/codeflowmu-six-ai-20260909/assets/00-instrument.png)](/articles/codeflowmu-six-ai-20260909/assets/00-instrument.png)

在这套测试流程中，CodeFlowMu承担三项工作：组织PM与工人角色协作；持续取得任务、执行和交付证据；提供独立EVAL及事后核查入口。后文所有计时、交付和质量判断，都沿这条记录链进行。六家模型与接入方式统一列在下一节。

### 1.4 接入方式与团队模型配置 {#section-1-4}

“六个AI”在本文指六家模型在各自接入配置下组成的团队。我们按Codex、豆包、DeepSeek、Kimi、千问、Cursor的顺序测试；名称与实际模型标识来自本轮身份回答和保存的运行记录。

| 被测模型 | 本轮模型标识 | 接入通道与执行框架 |
|---|---|---|
...
[![CodeFlowMu团队模型页面：团队成员与独立EVAL通道配置](/articles/codeflowmu-six-ai-20260909/assets/scene-team-models.png)](/articles/codeflowmu-six-ai-20260909/assets/scene-team-models.png)
...
### 1.6 CodeFlowMu，让一次聊天变成可检查的工作 {#section-1-6}

这次实验实际使用CodeFlowMu组织六轮团队巡检。它的贡献不是给模型加一个聊天窗口，而是把任务、角色、执行、交付和授权放进一条可追踪的工作流程。AI的能力因此有了可观察的落点。

[![CodeFlowMu，让一次聊天变成可检查的工作](/articles/codeflowmu-six-ai-20260909/assets/14-codeflow-contribution.png)](/articles/codeflowmu-six-ai-20260909/assets/14-codeflow-contribution.png)

本轮实测的产品价值：能追查谁接单、谁执行、谁交付，以及失败后怎样恢复。记录能力不等于所有投影均正确。

#### 把“会回答”变成“会组织工作”

ADMIN用相同正文下达任务，PM自行分配DEV、OPS和QA，工人提交正式报告，PM验收并汇总。CodeFlowMu提供了共享的工作结构，让六家模型的派单方式、等待关系、交付完整度和范围控制真正显出差别；最终答案相似，也不代表中间执行同样可靠。
...
### 1.7 文件才是真相：记录必须落地，不能只靠记忆 {#section-1-7}
...
| 独立观察 | 任务记录、系统观察与关单旁观材料，以及生成失败记录和迟到补充材料 | 提供独立判断及证据缺口，继续接受事实核查 |
...
[![文件才是真相：记录必须落地，不能只靠记忆](/articles/codeflowmu-six-ai-20260909/assets/15-evidence-chain.png)](/articles/codeflowmu-six-ai-20260909/assets/15-evidence-chain.png)
...
### 1.8 Agent工作之后：CodeFlowMu的一整套业务支持 {#section-1-8}

**CodeFlowMu不仅让Agent开始工作，还支持交付后的证据核查、角色验收、独立观察和故障诊断。** 这些环节依靠实际落地的文件相互关联，构成我们评判团队工作的业务基础。运行记录、程序核查、PM判断和EVAL分析不能混成一句“系统认为完成”。

[![CodeFlowMu从执行、交付到核查、验收、EVAL和诊断的业务支持](/articles/codeflowmu-six-ai-20260909/assets/18-business-support.svg)](/articles/codeflowmu-six-ai-20260909/assets/18-business-support.svg)
...
### 1.10 单独看EVAL：由另一个Agent观察，三类报告各回答一个问题 {#eval-report-matrix}

**执行团队说“做完了”之后，谁来检查这句话？在CodeFlowMu里，是执行团队之外的另一个EVAL Agent。** PM负责组织交付，DEV、OPS、QA承担具体工作；EVAL在独立会话里打开这些角色留下的文件和执行记录，比较报告中的声明与实际证据，形成另一份分析意见。

本次六轮均使用独立的Cursor SDK / `auto-smart`评估配置。这里的“独立”首先指角色、会话和职责独立：即使被测团队同样使用Cursor，EVAL仍是另一个评估会话；它不代表一定使用了不同的基础模型，也不保证不会判断失误。统一配置有助于比较，证据复核仍然必要。

#### 任务一开始就记录，分析在生成报告时进行

ADMIN在任务创建时选择记录后，CodeFlowMu建立运行编号，保存根任务、环境与模型配置、开始时间和日志起始游标。任务执行期间，系统沿用已有的聊天、公开过程、工具、会话和业务日志持续留证；**这不等于EVAL模型从头到尾一直运行、对每一步实时分析。**

结束后触发“生成任务记录”，系统冻结结束边界，递归关联完整子任务树，汇集本轮材料，再启动独立EVAL分析。这样既能从任务开始追溯过程，又把持续留证与模型分析的调用成本分开。未选择记录的运行不能事后冒充为完整的预先记录；有原始记录，也不等于已成功生成最终分析报告。

#### 三类报告矩阵：过程、系统、交付声明

| 报告 | 触发与观察范围 | 独立分析主要回答什么 | 典型文件标识 |
|---|---|---|---|
| **任务记录报告** | 创建任务时启用记录；生成时冻结一轮根任务及其完整子任务树 | PM怎样派单，团队怎样执行，耗时、重试、人工介入和结果怎样；本轮关联ISSUE是否有证据、怎样处理 | `*-benchmark-<RUN-ID>.md` |
| **系统观察报告** | EVAL页面的“生成观察报告”等观察入口；读取当时系统资产快照 | 九类资产是否一致，存在什么风险、证据缺口或涌现结构，哪些内容值得沉淀 | `*-panel-scan.md` |
| **关单旁观报告** | 根任务的EVAL旁观入口；围绕PM向ADMIN提交的最终报告及其任务链 | PM与工人的具体声明是否被执行证据支持，有无矛盾或未验证项，ADMIN验收前应关注什么 | `*-eval-agent.md` |

三者都有分析，但分析对象不同。记录报告重建**这一轮怎样完成或失败**；观察报告检查**系统资产呈现出什么状况**；关单旁观报告核查**这份交付报告说得是否有依据**。它们不是三份改名后的相同总结，也不是自动串行执行的三个审批关卡。

系统观察的九类资产是：账本、运行日志、公开思考流、用量、统计、内部EVAL、涌现日志、角色视图和共享知识。公开思考流只指系统实际收到并保存的过程内容，不能用它推测未输出的隐藏推理。

#### 哪些技能支撑这些分析？

技能是交给EVAL Agent的工作规范，不是额外的模型，也不是一项技能对应一份报告。测试版本`cb590ce`的实际路由共组合了以下七项技能；“✓”表示该报告路径要求注入，不能据此认定某轮已经正确完成分析。

| 技能与作用 | 任务记录 | 系统观察 | 关单旁观 |
|---|:---:|:---:|:---:|
| `eval-statistical-analysis`：锁定运行身份，统计耗时、调用、失败与重试，保留公式、分母和来源 | ✓ | — | — |
| `eval-issue-analysis`：逐项核查本轮关联ISSUE，分类、评估证据、分析影响和原因置信度 | ✓ | — | — |
| `eval-observation-writing`：把事实、推断、缺口和建议分开，引用来源并按报告合同写作 | ✓ | ✓ | — |
| `eval-admin-closeout-observer`：检查PM终报、工人报告与当前有效任务证据链的一致性 | — | ✓ | ✓ |
| `controlled-emergence-observer`：依据任务关系观察probe、自建任务、sandbox与项目树结构 | — | ✓ | — |
| `eval-risk-gap-analysis`：比较预期与实际，说明风险、严重度建议和责任归属 | — | ✓ | — |
| `eval-promotion-advice`：建议哪些发现可转后续任务、问题草稿或共享资产 | — | ✓ | — |

其中，`eval-statistical-analysis`专门约束“怎么算、怎样解释”：先确认任务树与时间窗口，再归一化、去重和统计；区分执行时间与等待、任务完成与产品QA通过；证据缺失写`unknown`，证据冲突写`disputed`。评分是可选项，必须公开公式，不能用高分掩盖未完成或证据不足。

系统观察路径也注入关单证据核对技能，但报告范围仍由`system_observation`合同限定；这不意味着它同时生成了另一份关单旁观报告。仓库另有`fcop-eval-promotion`辅助规范，用于后续分类和内部草稿处理，**它不在上面三条生成路径的必需技能清单中**。晋升建议不等于自动创建任务、提交公开Issue或修改生命周期。

#### EVAL记录矩阵：日志留证、形成资产、事实判断

这套能力可以沿三个层次阅读：**日志留证**保存实际发生的事情；**资产整理**把任务、报告、证据包和索引变为可追溯的分析材料；**事实核查与分析判断**由独立EVAL Agent核对声明、识别矛盾、说明不确定性并提出建议。原始日志主要由CodeFlowMu与Host产生，不能全部归为EVAL生成；EVAL的独立分析也会成为后续可复核、可积累的文件资产。

这里的“事实判断”是有来源、可质疑的分析，不是自动认定最终真相，也不是替ADMIN验收。七项技能分别约束统计、问题分析、写作、交付核查、涌现观察、风险分析和沉淀建议，把这个矩阵落实到具体工作。

| 留存层 | 文件或字段 | 能证明什么 |
|---|---|---|
| 起始记录 | `.codeflowmu/eval-recordings/<RUN-ID>.json` | 本轮记录身份、起止边界、任务关联与生成状态 |
| 运行材料 | `research/evidence/benchmarks/.../<RUN-ID>/`中的`raw/`、`artifacts/`及索引 | EVAL和事后分析实际可以读取哪些原始材料 |
| 程序采集 | 任务记录材料、资产扫描、`*-evidence-bundle.md` | 采集程序整理了哪些事实；这些材料本身不是独立Agent结论 |
| 独立分析 | EVAL Session、模型来源、`analysis_skill_ids`或`skill_ids`及技能回执 | 哪个评估会话获得了哪些技能；注入回执证明加载，不证明判断正确 |
| 最终报告 | `fcop/internal/eval/`中的分析正文及对应Agent、Session、运行来源字段 | 独立分析实际生成并落地，结论可沿引用重新核查 |

生成时，EVAL Agent读取材料并返回分析文本，Runtime负责检查格式、来源及相应技能回执，再写入报告。程序快照、Agent分析和落盘结果是三个不同环节。因此，一个页面出现文件、一次会话显示完成，或者记录状态变为“已生成”，都不能单独证明完整分析已经合格落地。具体源码片段见后文[3.9 EVAL如何生成并核验报告](#section-3-9)。

**这正是CodeFlowMu在测试中的作用：既留下执行团队的工作证据，又提供另一个Agent读取、质疑和分析这些证据的业务路径。** EVAL给出独立意见，PM承担交付职责，ADMIN保留验收与后续处置权。本文再对照分轮备份复核EVAL；三类报告路径的存在，不代表本次每一轮都成功取得三份报告。缺失、失败、重试和材料错配仍按实记录。

本节机制依据为测试提交`cb590ce`中的`codeflowmu-shell/src/eval-independent-analysis.ts`（三类路径与必需技能）、`eval-benchmark-recording.ts`、`packages/evaluator/eval-report-writer.js`和`EvalObservationGenerator.ts`。这是对实现机制的说明，不把源码存在等同于某轮执行成功。

### 1.11 为什么是九类资产？EVAL具体看什么 {#eval-nine-assets}

**九类来自CodeFlowMu系统观察器的实际资产清单，目的是把同一件事在不同记录里的表现对照起来。** 它不是九个Agent，也不是九个评分维度，更不是对所有系统都通用的固定分类。测试版本的`ASSETS_ANALYZED`明确列出以下九类；TASK、REPORT、ISSUE等业务文件则通过账本、运行材料和报告引用进入核查。

| 资产 | 主要记录 | EVAL怎样利用它分析 | 需要回答的问题 |
|---|---|---|---|
| **1. 账本 `ledger`** | 任务登记、角色路由、父子关系、状态与业务记录关联 | 重建正式任务结构，对照生命周期和交付记录 | PM到底派了哪些任务？哪些已经有正式交付，哪些仍未收口？ |
| **2. 运行日志 `runtime`** | Session、attempt、lease、工具调用与返回、执行错误 | 核对实际执行、重试、失败和状态变化 | “已经执行”有没有返回结果？会话结束是否真的交付了报告？ |
| **3. 公开思考流 `thinking`** | 模型实际输出的计划、过程说明和工具活动 | 比较当时的声明与后续行为，观察范围变化和解释过程 | PM说要派单后有没有派？何时改变了计划？不推测未输出的隐藏推理 |
| **4. 用量 `usage`** | 系统实际采集的请求、Token和用量记录 | 按可关联的运行范围核对消耗及记录缺口 | 消耗属于哪一轮？是否包含重试和其他会话？没有账单关联就不能当成精确任务费用 |
| **5. 统计 `analytics`** | 聚合事件、计数与时间指标 | 对照原始事件，核查计数、去重、分母和统计窗口 | “调用多少次、耗时多久”的计算口径是否一致？ |
| **6. 内部EVAL `eval`** | 历史观察、采集材料、独立分析及生成状态 | 核对评估身份、来源、分析是否实际落地和材料是否错配 | 是完整Agent分析，还是只有采集摘要？是否引用了另一轮的材料？ |
| **7. 涌现日志 `emergence-log`** | 已记录的结构模式、来源、风险和建议 | 对照当前任务关系，追踪已有观察是否重复、演化或失去证据支持 | 新发现是否已经记过？观察到了什么变化，有什么可复用价值？ |
| **8. 角色视图 `views`** | PM、DEV、OPS、QA等角色的待办与状态投影 | 对照账本、生命周期和视图合同 | 页面显示“运行中”是否与正式状态一致？待办为空是否符合当前阶段？ |
| **9. 共享知识 `shared`** | 团队共享的规则、经验、知识与复用材料 | 检查已有知识能否解释问题、是否过时，以及哪些发现值得沉淀 | 有可复用的经验却没有使用，还是缺少应保存的知识？ |

它的价值在于**跨资产核查**。例如，PM报告写“工具成功”，运行日志却只有调用开始，需要补返回证据；页面写“运行中”，正式取消收据与会话状态却显示已结束，需要核查投影；统计声称多次失败，用量和Runtime又能帮助区分模型请求、工具重试与同一事件的重复记录。

系统扫描报告为每类资产保留“**状态、关键发现、风险/价值、证据**”四个观察字段，再由独立EVAL分析跨资产矛盾、证据缺口与建议。“九类均已扫描”证明覆盖了采集清单，不能直接改写成“九类都已验证正常”。这也是CodeFlowMu把日志转成分析资产的关键：不只保存文件，还让同一事实可以从不同记录交叉检查。

### 1.12 涌现观察：识别协作中形成的结构，而不只统计错误 {#eval-emergence}

EVAL还观察：团队运行时，是否形成了值得记录的任务来源或组织结构。这里的“涌现”有明确的实现范围，**不是宣称模型突然获得了新智能**，也不能把普通报错都叫作涌现。

| 观察方向 | 识别依据 | 分析重点与价值 |
|---|---|---|
| **受控涌现 `controlled_emergence`** | 结合角色路由与明确的probe、self-task、sandbox自举标记，识别测试或探针产生的任务来源 | 说明这些任务从哪里来，是否混入正式待办、审核或统计；保留探针证据，避免把测试产物当成业务交付 |
| **项目树涌现 `project_tree_emergence`** | 在同一线程中，根据实际父子关系与角色链识别“主线任务 → 阶段任务 → 执行任务”结构 | 观察团队是否把任务协作组织成项目与阶段结构；结合交付结果判断价值，并检查错挂父任务、跨线程、环路及父任务已关闭但子任务未结束等风险 |

```text
普通分工：ADMIN根任务 → PM派发DEV / OPS / QA检查
项目树结构：ADMIN主线 → 阶段任务 → 各角色执行任务
```

标题里出现“Phase”“project”或“probe”，或者PM多创建几个子任务，都不足以证明命中这些模式。检测器需要核对任务来源、`parent`、`thread_key`、角色路由和相关标记；独立EVAL再检查解释是否成立，给出替代解释、置信度、风险与建议。`controlled-emergence-observer`是前面七项技能之一，项目树由配套的`project-tree-observer.js`检测器提供结构证据，它不是额外的一份必生成报告。

涌现观察既看风险，也看价值：探针任务混入业务视图可能造成误判；合理的阶段结构可能体现可复用的组织方式。但结构形成不等于业务成功，仍需对照执行与交付。相关观察写入系统观察报告，并由系统保留涌现日志，形成以后可以追踪、复核和沉淀的材料；EVAL只给建议，不自行清理任务、归档或改变派单。

**没有命中涌现模式，也是一项应保留的观察结果。** 本次巡检中正常的ADMIN→PM→DEV/OPS/QA分工不能仅凭“团队协作完成”就宣传为新涌现；各轮是否命中，应以该轮报告与任务关系证据为准。这使CodeFlowMu既能记录成功与故障，也能积累协作结构如何形成、是否有价值的观察，而不为增加亮点夸大结论。

本节依据测试版本的`packages/evaluator/eval-report-writer.js`、`controlled-emergence-observer.js`及`project-tree-observer.js`；九类资产是实现清单，上表的提问用于解释其分析用途，不代表每轮都已完成表内每项验证。


## 二、总的测试结果与每家AI的表现 {#chapter-2}

下面的结果由CodeFlowMu留下的正式任务、报告、执行与取消记录核对得出。先看团队是否交付，再看各家PM怎样组织工作、报告能支持什么结论；流程状态、检查质量和接入故障分别解释。

### 2.1 六家结果总表 {#section-2-1}

四家模型完成了三角色报告与PM汇总；Kimi未建立下游任务，千问虽进入团队执行但未完成全部交付。正常完成也存在报告质量差异，因此同时比较完成度、结果质量、效率、范围控制、恢复与可观察性。

| 模型 | 正常交付 | 工人正式交付 | 执行或终止耗时 | 综合分 / 100 | 本轮主要表现 |
|---|---|---|---|---|---|
...
[![六家模型的本轮综合评分](/articles/codeflowmu-six-ai-20260909/assets/01-scores.png)](/articles/codeflowmu-six-ai-20260909/assets/01-scores.png)

结果图｜评分只描述这六次集成运行，不是基础模型通用能力排名。每家当天一个正式样本。Codex和Cursor相差1分，应看作同一档。

评分权重：完成度25、结果质量30、效率15、范围控制15、恢复10、可观察性5。各维相加得总分；质量评价看证据是否支撑结论，效率评价结合交付与耗时，而不是只按秒数排序。明细见[评分数据](/articles/codeflowmu-six-ai-20260909/scorecard.csv)。

### 2.2 四轮交付完成 两轮被迫停止 {#section-2-2}

真正拉开差距的，是从接到任务到形成可信报告的全过程。仅看最后一段回答，会漏掉工具失败、人工介入和没有落盘的交付物。

[![四轮交付完成 两轮被迫停止](/articles/codeflowmu-six-ai-20260909/assets/01-scores.png)](/articles/codeflowmu-six-ai-20260909/assets/01-scores.png)
...
### 2.3 Codex：完成快，知道哪些结论需要保留 {#section-2-3}
...
### 2.4 豆包：能顺利组织团队，PM复核仍偏弱 {#section-2-4}
...
### 2.5 DeepSeek：取证有深度，但解释并非都可靠 {#section-2-5}
...
### 2.6 Kimi：本轮未通过团队执行的接入关 {#section-2-6}
...
### 2.7 千问：调查越做越深，最终没有完成团队交付 {#section-2-7}
...
### 2.8 Cursor：一次授权恢复，展示了协作记录的作用 {#section-2-8}
...
[![Cursor任务页面保留旧QA任务与模型修复后的新任务](/articles/codeflowmu-six-ai-20260909/assets/scene-cursor-recovery.png)](/articles/codeflowmu-six-ai-20260909/assets/scene-cursor-recovery.png)
...
### 2.9 百分制里 完成只是四分之一 {#section-2-9}

如果只看任务变成done，四个完成者会并列；如果只看时间，报告中的误判又会被忽略。评分必须同时回答交付、质量与管理问题。

[![百分制里 完成只是四分之一](/articles/codeflowmu-six-ai-20260909/assets/07-score-matrix.png)](/articles/codeflowmu-six-ai-20260909/assets/07-score-matrix.png)
...
[![相同任务没有变成相同任务图](/articles/codeflowmu-six-ai-20260909/assets/03-task-graphs.png)](/articles/codeflowmu-six-ai-20260909/assets/03-task-graphs.png)
...
### 3.2 看到配置 不等于验证能力 {#section-3-2}

不同团队拉开质量差距的地方，往往不是读了多少文件，而是能否分清一条证据究竟支持到哪一层。

[![看到配置 不等于验证能力](/articles/codeflowmu-six-ai-20260909/assets/04-evidence-layers.png)](/articles/codeflowmu-six-ai-20260909/assets/04-evidence-layers.png)
...
### 3.3 多花时间 有没有换来更多有效信息 {#section-3-3}

计时从PM首个正式会话开始，到最终报告成功写入结束。交付后的ADMIN验收、归档和EVAL生成另计；中途授权与恢复计入总用时。失败轮只列终止耗时。

[![多花时间 有没有换来更多有效信息](/articles/codeflowmu-six-ai-20260909/assets/02-duration.png)](/articles/codeflowmu-six-ai-20260909/assets/02-duration.png)
...
### 3.4 多花的时间 没有等量换来可信结论 {#section-3-4}

综合看四个正常交付者，Codex的速度优势没有伴随更低的质量评分。DeepSeek比它多用约27分钟，确实增加了运行取证，却未交出更可信的综合结论。效率应看新增证据带来的判断收益，而不是活动数量。

[![多花的时间 没有等量换来可信结论](/articles/codeflowmu-six-ai-20260909/assets/11-quality-time.png)](/articles/codeflowmu-six-ai-20260909/assets/11-quality-time.png)
...
### 3.5 千问的两小时 花在了哪里 {#section-3-5}

“没有发现问题”不符合证据；“因为发现了问题所以耗时合理”也不成立。要把前段等待、真实故障和后段管理失控拆开。

[![千问的两小时 花在了哪里](/articles/codeflowmu-six-ai-20260909/assets/05-qwen-timeline.png)](/articles/codeflowmu-six-ai-20260909/assets/05-qwen-timeline.png)
...
### 3.6 Cursor如何让失败的QA重新交付 {#section-3-6}

这一轮同样经过系统初始化，但QA槽位错误地选了Cursor SDK不可用的qwen3.8-max。这是模型配置异常；其后的恢复过程，比单看最终成功更有价值。

[![Cursor如何让失败的QA重新交付](/articles/codeflowmu-six-ai-20260909/assets/06-cursor-recovery.png)](/articles/codeflowmu-six-ai-20260909/assets/06-cursor-recovery.png)
...
### 3.7 接入Codex 会改变测试条件 {#section-3-7}
...
[![接入Codex 会改变测试条件](/articles/codeflowmu-six-ai-20260909/assets/12-integration-path.png)](/articles/codeflowmu-six-ai-20260909/assets/12-integration-path.png)
...
### 3.8 评估器也需要被检查 {#section-3-8}

EVAL统一采用Cursor入口，并不意味着每次输入、实际模型或结论都相同。本轮两类问题尤其影响文章能否作为测试证据发表。

[![评估器也需要被检查](/articles/codeflowmu-six-ai-20260909/assets/08-eval-mismatch.png)](/articles/codeflowmu-six-ai-20260909/assets/08-eval-mismatch.png)
...
### 3.9 EVAL到底评了什么？ {#section-3-9}

本次EVAL统一采用Cursor入口。下面重点对照面板扫描与任务记录这两条报告路径；此外，根任务还有针对PM终报证据链的关单旁观路径，三者分工见[1.10 EVAL矩阵](#eval-report-matrix)。程序采集段、独立分析段必须分开阅读；程序显示“已生成”也不等于独立分析已完成。
...
[![Cursor轮次最终生成的EVAL任务记录与面板扫描](/articles/codeflowmu-six-ai-20260909/assets/scene-eval-report.png)](/articles/codeflowmu-six-ai-20260909/assets/scene-eval-report.png)

点击截图查看原始像素，避免缩略图影响阅读。

现场图3｜用户提供的测试现场截图。报告成功落地后，仍需检查其内容和运行身份是否对应。

### 3.10 EVAL不只是附录，也不是最终真理 {#section-3-10}

文章的判断应同时回答两件事：AI有没有完成工作，证明这件事的材料靠不靠谱。EVAL最有价值的作用，是对照原始记录挑战看起来完整的报告；它自身的推断也必须接受复核。

[![EVAL不只是附录，也不是最终真理](/articles/codeflowmu-six-ai-20260909/assets/17-eval-judgment.png)](/articles/codeflowmu-six-ai-20260909/assets/17-eval-judgment.png)
...
### 3.11 事实核查：报告中的一句话，怎样回到原始记录 {#section-3-11}
...
### 3.12 诊断：把“卡住了”分解成可处理的问题 {#section-3-12}
...
### 3.13 两轮未完成：停止位置、原因与判断边界 {#section-3-13}
...
## 四、最终结论：AI表现与CodeFlowMu、FCoP的价值 {#chapter-4}

### 4.1 CodeFlowMu让这次比较有据可查 {#section-4-1}
...
只依据这轮证据，我会优先选Codex承担常规巡检，把Cursor作为有人工审批条件下的同档选择；国产模型中优先复测DeepSeek，其次豆包。千问需要先验证任务控制，Kimi需要先验证接入。这个取舍同时考虑交付、可信度、耗时和干预成本。

[![综合判断](/articles/codeflowmu-six-ai-20260909/assets/10-overall-verdict.png)](/articles/codeflowmu-six-ai-20260909/assets/10-overall-verdict.png)
...
### 4.3 国产模型能做事，稳定交付还要分档 {#section-4-3}

这轮支持的判断是：豆包与DeepSeek已具备在当前框架里分解任务、使用工具、形成团队报告的能力；它没有证明国产模型整体不足，也没有证明它们已能替代需要严格复核的PM。能跑通、报告可信、遇错会恢复，是三道不同门槛。

[![国产模型能做事 稳定交付还要分档](/articles/codeflowmu-six-ai-20260909/assets/13-decision.png)](/articles/codeflowmu-six-ai-20260909/assets/13-decision.png)
...
### 4.4 可靠的Agent 也要知道何时结束 {#section-4-4}

检查不应以消灭所有缺陷为终点。发现异常、给出可靠证据和下一步建议，就可以完成巡检；修复需要另一份清楚的授权。

[![可靠的Agent 也要知道何时结束](/articles/codeflowmu-six-ai-20260909/assets/09-baseline.png)](/articles/codeflowmu-six-ai-20260909/assets/09-baseline.png)

图9｜建议的下一轮流程。导出并校验保护旧证据，初始化后核对基线保护新实验；两者解决不同问题。此图为后续实验建议，非今天已执行的全部步骤。

#### 原题保留 另开明确边界的实验线

A线继续用同一原题，观察自然语言理解；B线明确只读、允许正式派单和报告、不授权业务修复、30分钟内力争汇总。不能把两线混在一起宣布模型改善。建议轮换顺序、各重复至少三次，并分别保存任务记录、系统观察及实际触发的关单旁观报告与失败草稿。
