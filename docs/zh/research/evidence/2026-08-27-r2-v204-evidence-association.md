...
本页记录一条完整的研究—工程链路：固定历史 REPORT 样本先暴露“生命周期位置不能证明证据归属”；随后 R2 被实现为只读证据关联诊断；真实任务又暴露诊断器本身的假阳性；V2.0.4 收紧语义后，同一任务从 `active` 进入 `review` 时，诊断能够按当前正式事实动态重算。
...
公开文章不复制私有生产源码；这里只披露与文章主张直接相关的语义合同。

## 3. 真实反例：第一版诊断器自己产生假阳性

真实任务 `TASK-20260827-024` 暴露过四类假阳性：revision mismatch、REPORT ownership conflict、execution missing、formal REVIEW missing。
...
- 该证据支持“研究结论已进入真实工程并经历反例纠偏”的案例，不认证全部任务、生命周期组合、桌面端或 PWA 路径。
