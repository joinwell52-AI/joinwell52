# Publication Center

joinwell52 Research Center 3.0 publishes versioned Runtime capabilities, Digital Employee capabilities, papers, specifications and engineering case reports. GitHub is the single source of truth; a revision is official only after the applicable Runtime Gate, Git commit and commit verification.

## Research Center 3.0 Runtime release

| Type | Document | Version | Status |
|---|---|---:|---|
| Runtime control plane | [Research Runtime Scheduler](./research-runtime-scheduler-v1.0) | V1.0 | Released |

Research Runtime Scheduler V1.0 is the only formal scheduler and observability layer of the Research Operating System. It governs seven Runtime tasks, daily Runtime Records, the generated Runtime Center and the publication gate.

## Digital Research Employee capability

| Type | Document | Version | Status |
|---|---|---:|---|
| Digital Employee capability | [Research Report Production Engine](./research-report-production-engine-v1.0) | V1.0 | Production Verified |

The Research Report Production Engine is the ChatGPT-based Research Analyst worker. In Research Center 3.0 it operates under Research Runtime Center and closes official work through Runtime Records and GitHub verification.

## TMPA publication set

| Type | Document | Version | Status |
|---|---|---:|---|
| Paper | [TMPA Architecture Paper](./tmpa-architecture-paper-a0.4) | A0.4 | Public research draft |
| Specification | [TMPA Core Specification](./tmpa-core-specification-s0.3) | S0.3 | Public specification draft |
| Case report | [TMPA–FCoP–CodeFlowMu Implementation Case](./implementation-case-i0.3) | I0.3 | Public engineering case draft |

```text
Research Runtime
→ Runtime Record
→ bilingual publication
→ GitHub branch and commit
→ CI validation
→ main
→ Commit Verify
→ Pages build
→ Research Center
```

Until a stable release or DOI exists, citations should include author, title, explicit version, repository URL and access date.
