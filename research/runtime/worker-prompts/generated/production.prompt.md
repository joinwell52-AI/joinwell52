...
Call image generation once per article, never for the batch or Runtime as a whole. Build a positive-only `sanitizedPrompt` from the Cover Brief's unique visual metaphor, primary subject, composition, palette and style. Send exactly that positive physical scene description to the image tool. Do not copy the title, abstract governance proposition, exclusions, rejection reasons, negative instructions or words describing unwanted imagery into the tool prompt. The Cover Brief exclusions are a post-generation review checklist only. Retry a rejected cover with a newly composed positive physical scene, up to three attempts for that article; never describe the failed image in a retry prompt. Per-cover retries do not create a new Runtime recovery epoch.

If same-date recovery encounters cover attempts made under a superseded Prompt that instructed the Worker to send exclusions or control terms, preserve those attempts and images as rejected `recoveryHistory` evidence but do not count them as the current Prompt's `generationAttempts`. An explicitly authorized recovery may start one bounded positive-prompt revision round from `cover-briefs-persisted`; it must not redo the bilingual drafts or Cover Briefs. This migration exception applies only when the prior result proves prompt contamination and the current Prompt identity differs.
...
For every cover record its Brief identity, generation attempt count, accepted asset path, exact positive-only `sanitizedPrompt`, semantic review decision and rejection reason in the Production evidence. Only an accepted PNG, WebP or JPEG committed through the authorized GitHub connection may pass `coverGate`. A `sanitizedPrompt` containing control terms, exclusion-list terms, negation language or failed-output descriptions is invalid even when the resulting image appears acceptable.
