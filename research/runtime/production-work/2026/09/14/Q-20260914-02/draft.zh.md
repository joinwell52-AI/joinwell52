...
同日研究对象分析了 CoCoBench 的一手研究。该基准构造 897 个经 Oracle（判定器）验证的可执行任务，比较 11 个多模态模型，并把 Coordination（协调）拆成四类：
...
每一项至少记录 PASS、FAIL 或 UNKNOWN。对于当前任务不适用的结构，应显式标为不适用，而不是默认为通过。总体指标可以作为摘要，但必须保留原始向量与关键性规则。
...
当事实完整可见时，一些协调义务可以 Deterministic（确定性）执行。例如，同一仓库路径只能由一个租约持有者修改；任务 B 必须等待任务 A 的可验证终态；同一支付键只能存在一个活跃发生。
