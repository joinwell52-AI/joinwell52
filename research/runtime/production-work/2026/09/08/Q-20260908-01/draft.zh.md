...
![记忆还在不等于还能继承的题图](staging/publication-candidates/2026-09-08-memory-must-be-readmitted-cover.png)
...
| 状态 | 它回答什么 | 不能代替什么 |
|---|---|---|
| 存储完整 | 数据是否存在、可解析、形态正确 | 新系统是否仍理解相同意义 |
| 语义兼容 | 具体迁移方向是否通过来源有据的语义探针 | 出错后是否能恢复 |
| 可恢复 | 是否保留允许使用的来源，以及修复程序是否在该方向上测试过 | 是否已经允许启用 |
| 激活授权 | 在前述证据和政策条件满足后，是否允许新运行环境使用这代记忆 | 兼容性的技术证明 |
...
- [智能体记忆可移植性主研究](https://arxiv.org/html/2609.05339v1)
