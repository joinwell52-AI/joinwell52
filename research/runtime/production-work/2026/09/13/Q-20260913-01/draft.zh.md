...
报告的基准结果也支持这种局部恢复价值。在主要比较中，GPT-5.4（生成式预训练变换器模型版本）采用该方法后，任务成功率从继续执行条件下的 62.2% 提升到 87.8%，最长检查项进度从 81.4% 提升到 94.3%；较小模型的对应指标也改善。Terminal-Bench 2.0（终端任务基准第二版）的泛化比较同样有利于对齐回退。
...
- [《AgentRewind：面向长时程大语言模型智能体的可恢复执行》](https://arxiv.org/abs/2608.14380)，一手研究论文，2026 年。
- [replay-agent-recorder（智能体执行记录器）参考实现](https://github.com/Futuresis/replay-agent-recorder)，公开一手代码；其实现事实不是独立验证。
- [MettleBench（长时程工程任务基准）](https://github.com/Kelvin-Coffee/MettleBench)，公开基准仓库。
