...
同日研究对象分析了 LifeFuse-Mem（生命周期记忆框架）及其 Hard Attribution Anti-Overwrite（强归因抗覆盖）基准。该实验包含一千个片段：先引入一条长期事实，再引入与它冲突的临时事实。系统在写入前已经知道每条内容的生命周期，并把在线记忆分成较稳定与较可塑的子空间。
