...
![证据照亮行动路径，但独立的执行闸门仍决定是否能够越过边界](staging/publication-candidates/2026-09-12-evidence-complete-not-execution-authority-cover.png)
...
对比研究 Bounded Agents 展示了另一层问题：权威会沿主体链委派，并受累计预算、既往动作和组合限制约束。父级拥有的能力不能简单复制给子级；一次批准也不应泛化成对相似动作的长期通行证。
