...
function object(overrides = {}) {
  ...
  const value = {
    ...
    lifecycle: { profile: 'tmpa-s0.5-case-profile', state: overrides.state ?? 'created' },
    references: structuredClone(overrides.references ?? []),
    content: { media_type: 'application/json', body: structuredClone(overrides.body ?? {}) },
    integrity: { canonicalization: 'tmpa-s0.5-stable-json-1', hash_algorithm: 'sha256', digest: '' }
  }
...
export const tests = [
  ...
  },
  {
    ...
    run(read) {
      ...
      return { result, context: { issue_codes: issueCodes(result), review_judgment: result.nodes.find((node) => node.id === review.id)?.judgment, state: result.extensions.summary.work_state['work-c07'], risk_codes: issueCodes(riskResult), risk_view: riskResult.view_state } }
    }
  },
  {
    id: 'C08',
