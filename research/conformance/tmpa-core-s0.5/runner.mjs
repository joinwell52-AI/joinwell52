...
const corpus = path.join(root, 'research/conformance/tmpa-core-s0.5')
const artifacts = path.join(corpus, 'artifacts')
const executedAt = process.env.TMPA_EXECUTED_AT ?? '2026-08-05T09:00:00Z'
...
const schemaPaths = {
  objectSchema: 'docs/public/spec/tmpa/s0.5/governance-object.schema.json',
  lifecycleSchema: 'docs/public/spec/tmpa/s0.5/lifecycle-profile.schema.json',
  readerSchema: 'docs/public/spec/tmpa/s0.5/reader-result.schema.json',
  conformanceSchema: 'docs/public/spec/tmpa/s0.5/conformance-result.schema.json'
}
const sourcePaths = {
  fixtures: 'research/conformance/tmpa-core-s0.5/fixtures.mjs',
  readerImplementation: 'research/conformance/tmpa-core-s0.5/reader.mjs',
  runner: 'research/conformance/tmpa-core-s0.5/runner.mjs',
  profile: 'research/conformance/tmpa-core-s0.5/profile.json',
  productEvidence: 'research/conformance/tmpa-core-s0.5/product-evidence.json',
  fcopRun: 'research/conformance/tmpa-core-s0.5/external-runs/fcop-3.2.5-rerun.json',
  codeflowAvailability: 'research/conformance/tmpa-core-s0.5/external-runs/codeflowmu-v1.2.3-availability.json'
}
...
const profileBundle = {
  ...
  object_schema: versioned(objectSchema.$id, 'S0.5', sourceDigests.objectSchema),
  type_registry: versioned('tmpa-s0.5-case-types', '1.0.0', sourceDigests.profile),
  lifecycle_registry: versioned(profile.id, profile.version, sourceDigests.profile),
  role_registry: versioned('tmpa-s0.5-case-roles', '1.0.0', sourceDigests.profile),
  relation_registry: versioned('tmpa-s0.5-case-relations', '1.0.0', sourceDigests.profile),
  integrity_profile: versioned('tmpa-s0.5-stable-json-integrity', '1.0.0', sourceDigests.readerImplementation),
  canonicalization_profile: versioned('tmpa-s0.5-stable-json-output', '1.0.0', sourceDigests.readerImplementation)
}
...
for (const test of tests) {
  ...
  const manifest = {
    test_case_id: `S0.5-${test.id}-reference-001`,
    criterion: test.id,
    core_version: 'S0.5',
    object_schema: profileBundle.object_schema,
    output_schema: versioned(conformanceSchema.$id, 'S0.5', sourceDigests.conformanceSchema),
    ...
    runner: { id: 'tmpa-s0.5-conformance-runner', version: '0.1.0', command: 'npm run tmpa:s0.5:conformance' },
    environment: { node: process.version, platform: process.platform, arch: process.arch },
    permutation: test.id === 'C11' ? { method: 'complete', count: 24, seed: null } : { method: 'none', count: 1, seed: null },
    evidence_paths: {
      criterion_record: `research/conformance/tmpa-core-s0.5/artifacts/criteria/${test.id}.json`,
      reference_result: 'research/conformance/tmpa-core-s0.5/artifacts/reference-result.json',
      product_result: 'research/conformance/tmpa-core-s0.5/artifacts/product-result.json'
    }
  }
...
const referenceResult = {
  core_version: 'S0.5',
  implementation: { id: 'tmpa-s0.5-reference-reader', version: '0.1.0' },
...
const productResult = {
  core_version: 'S0.5',
  implementation: { id: 'fcop-3.2.5-codeflowmu-i0.5-baseline', version: 'S0.5-rerun' },
...
const summary = {
  ...
  claim_boundary: 'Reference fixtures demonstrate the S0.5 reader. The product aggregate is FAIL because the fcop/fcop-mcp v3.2.5 reference implementation failed the parent-field roundtrip test; this is not a failure of the FCoP protocol and does not establish TMPA Core S0.5 Conformance or independent adoption.'
}
await output('summary.json', summary)
await output('run-log.json', {
  executed_at: executedAt,
  command: 'TMPA_EXECUTED_AT=2026-08-05T09:00:00Z npm run tmpa:s0.5:conformance',
  environment: { node: process.version, platform: process.platform, arch: process.arch },
