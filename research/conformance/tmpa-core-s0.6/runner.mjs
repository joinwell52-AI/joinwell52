...
const corpus = path.join(root, 'research/conformance/tmpa-core-s0.6')
...
const schemaPaths = {
  objectSchema: 'docs/public/spec/tmpa/s0.6/governance-object.schema.json',
  lifecycleSchema: 'docs/public/spec/tmpa/s0.6/lifecycle-profile.schema.json',
  readerSchema: 'docs/public/spec/tmpa/s0.6/reader-result.schema.json',
  conformanceSchema: 'docs/public/spec/tmpa/s0.6/conformance-result.schema.json'
}
const sourcePaths = {
  fixtures: 'research/conformance/tmpa-core-s0.6/fixtures.mjs',
  readerImplementation: 'research/conformance/tmpa-core-s0.6/reader.mjs',
  runner: 'research/conformance/tmpa-core-s0.6/runner.mjs',
  profile: 'research/conformance/tmpa-core-s0.6/profile.json',
  canonicalizationProfile: 'research/conformance/tmpa-core-s0.6/canonicalization-profile.json',
  productEvidence: 'research/conformance/tmpa-core-s0.6/product-evidence.json'
}
...
const profileBundle = {
  ...
  object_schema: versioned(objectSchema.$id, 'S0.6', sourceDigests.objectSchema),
  type_registry: versioned('tmpa-s0.6-case-types', '1.0.0', sourceDigests.profile),
  lifecycle_registry: versioned(profile.id, profile.version, sourceDigests.profile),
  role_registry: versioned('tmpa-s0.6-case-roles', '1.0.0', sourceDigests.profile),
  relation_registry: versioned('tmpa-s0.6-case-relations', '1.0.0', sourceDigests.profile),
...
for (const test of tests) {
  ...
  const manifest = {
    test_case_id: `S0.6-${test.id}-reference-001`,
    criterion: test.id,
    core_version: 'S0.6',
    object_schema: profileBundle.object_schema,
    output_schema: versioned(conformanceSchema.$id, 'S0.6', sourceDigests.conformanceSchema),
    ...
    runner: { id: 'tmpa-s0.6-conformance-runner', version: '0.2.0', command: 'npm run tmpa:s0.6:conformance' },
    environment: { node: process.version, platform: process.platform, arch: process.arch },
    permutation: test.id === 'C11' ? { method: 'complete', count: 24, seed: null } : { method: 'none', count: 1, seed: null },
    evidence_paths: {
      criterion_record: `research/conformance/tmpa-core-s0.6/artifacts/criteria/${test.id}.json`,
      reference_result: 'research/conformance/tmpa-core-s0.6/artifacts/reference-result.json',
      product_result: 'research/conformance/tmpa-core-s0.6/artifacts/product-result.json'
    }
  }
...
const referenceResult = {
  core_version: 'S0.6',
  implementation: { id: 'tmpa-s0.6-reference-reader', version: '0.2.0' },
...
const productResult = {
  core_version: 'S0.6',
  implementation: { id: 'no-s0.6-product-baseline', version: 'not-run' },
...
const summary = {
  ...
  prior_version_evidence: { core_version: 'S0.5', report: 'I0.7', product: 'CodeFlowMu V1.4.1', verdict: '14 PASS / 0 FAIL', relabeled_as_s0_6: false },
  claim_boundary: 'The Reference Reader demonstrates the tested S0.6 paths. No S0.6 product run or independent validation is claimed. I0.7 remains exact-version S0.5 author-run evidence and is not relabeled.'
}
await output('summary.json', summary)
await output('run-log.json', {
  executed_at: executedAt,
  command: 'TMPA_EXECUTED_AT=2026-08-09T23:55:00Z npm run tmpa:s0.6:conformance',
  environment: { node: process.version, platform: process.platform, arch: process.arch },
