...
export function compareUnicodeCodePoints(leftValue, rightValue) {
...
export function stable(value) {
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.keys(value).sort(compareUnicodeCodePoints).map((key) => [key, stable(value[key])]))
  }
  return value
}
...
function compareTuple(a, b) {
  for (let index = 0; index < Math.max(a.length, b.length); index += 1) {
    ...
    if (left !== right) return compareUnicodeCodePoints(left, right)
  }
  return 0
}
...
function orderTransitions(candidates) {
  ...
  while (pending.size) {
    ...
    }).sort((a, b) => compareUnicodeCodePoints(a.object.id, b.object.id))
    const selected = ready[0] ?? [...pending.values()].sort((a, b) => compareUnicodeCodePoints(a.object.id, b.object.id))[0]
...
export function createReader({ objectSchema, readerResultSchema, lifecycleProfile, profileBundle }) {
  ...
  function read(sources) {
    ...
    })).sort((a, b) => compareUnicodeCodePoints(a.source_id, b.source_id))
    ...
    for (const source of sourceRecords) {
      ...
      if (!validateObject(object)) {
        addIssue('SCHEMA_INVALID', source.source_id, object.id ?? '', '', '', {
          errors: validateObject.errors.map((error) => `${error.instancePath}:${error.keyword}`).sort(compareUnicodeCodePoints)
        })
        continue
      }
    ...
    const accepted = []
    for (const [id, candidates] of [...groups.entries()].sort((a, b) => compareUnicodeCodePoints(a[0], b[0]))) {
      ...
      if (distinct.size > 1) {
        addIssue('DUPLICATE_ID_CONFLICT', candidates[0].source.source_id, id, '', '', {
          source_ids: candidates.map((candidate) => candidate.source.source_id).sort(compareUnicodeCodePoints)
        })
        continue
      }
      const orderedCandidates = candidates.sort((a, b) => compareUnicodeCodePoints(a.source.source_id, b.source.source_id))
      accepted.push({
        ...orderedCandidates[0],
        source_ids: orderedCandidates.map((candidate) => candidate.source.source_id)
      })
    }

    const byId = new Map(accepted.map((candidate) => [candidate.object.id, candidate]))
    const nodes = accepted.map(({ object, source, source_ids, canonical_digest }) => ({
      id: object.id,
    ...
    for (const [workId, candidates] of [...workGroups.entries()].sort((a, b) => compareUnicodeCodePoints(a[0], b[0]))) {
      const carriers = [...new Set(candidates.map((candidate) => candidate.object.governed_work.primary_carrier_id))].sort(compareUnicodeCodePoints)
    ...
    for (const [streamId, candidates] of [...streamGroups.entries()].sort((a, b) => compareUnicodeCodePoints(a[0], b[0]))) {
    ...
    for (const candidate of accepted) {
      ...
      if (requiresHuman) {
        const approved = object.references.some((reference) => {
          ...
          const assigned = approval && assignments.some((assignment) => (
            assignment.identity === approval.creator && assignment.role === approval.role
          ))
          const independent = approval && (
            !riskProfile.requires_independent_actor || approval.creator !== object.creator
          )
          return approval
            && riskProfile.approval_object_types.includes(approval.type)
    ...
    for (const [workId, state] of Object.entries(workState)) {
      ...
      if (openChildren.length) {
        addIssue('CHILD_WORK_OPEN', carrier.source.source_id, carrier.object.id, 'parent', '', { work_id: workId, open_children: openChildren.sort(compareUnicodeCodePoints) })
    ...
    for (const [target, members] of [...reviewGroups.entries()].sort((a, b) => compareUnicodeCodePoints(a[0], b[0]))) {
      ...
      if (!resolved) {
        addIssue('UNRESOLVED_CONFLICT', members[0].source.source_id, members[0].object.id, 'reviews', target, { reviews: [...memberIds].sort(compareUnicodeCodePoints) })
    ...
    const result = {
      ...
      reader: { id: 'tmpa-s0.6-reference-reader', version: '0.2.0' },
