...
![一个计算智能体已掌握蓝色方法结构，但琥珀色高风险动作仍停在独立授权边界，等待一次性批准](../../../staging/publication-candidates/2026-08-20-skill-vs-tool-authority-cover.png)
...
![四层分别回答怎样做、能调用什么、这次影响谁、是否允许这一次；匹配批准只获得一次受控执行尝试](../../../staging/publication-candidates/2026-08-20-skill-vs-tool-authority-figure-1.png)
...
这种分层也符合外部规范的边界。MCP 是连接大模型应用与工具的开放协议。其[安全与信任说明](https://modelcontextprotocol.io/specification/2025-03-26/index)明确指出，工具可形成任意数据访问和代码执行路径，工具描述与注解不能默认受信；宿主需要提供清晰的授权和用户控制。MCP 的[授权规范](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization)又进一步处理 HTTP 传输中的资源绑定、权限不足和令牌安全。

这些规范说明“工具连得上”与“当前操作被允许”不是一回事，但它们不替 CodeFlowMu 定义角色表，也不替本地 Runtime 判断某个文件路径是否逃出项目。
...
从通用安全概念看，角色能力门接近基于角色的粗粒度授权，操作策略接近基于属性和副作用的细粒度判断；MCP 负责连接工具，却不替宿主完成这两层授权。它们都不能替代容器、低权限进程、文件系统访问控制或网络出口限制。
