...
![开发者在 Cursor 中组织实现、环境验证和测试 Agent 完成同一项交付](03-visuals/cover-cursor-ai-team-final.webp)
...
![需求卡经 PM 拆分后由 DEV、OPS 和 QA 形成证据包，再由 PM 决策并交给人类批准](03-visuals/figure-cursor-accepted-delivery.png)

*图 1：执行者的 `done` 只是完成声明；独立测试、PM 接收或返工、EVAL 旁路观察和人工批准必须保持不同责任。*
