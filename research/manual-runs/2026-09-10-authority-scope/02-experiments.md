...
复跑：`python probe-ag2.py 3`。结果：[第1轮](runs/ag2-1.json)、[第2轮](runs/ag2-2.json)。
...
两轮各 10 个场景，结果一致。先前装置启动失败及修正见 [装置记录](runs/harness-notes.md)。我们没有真实 CLI 握手、SDK 服务端、认证、模型输出或秘密外泄证据。

复跑：`python probe-openhands.py 3`。结果：[第1轮](runs/openhands-1.json)、[第2轮](runs/openhands-2.json)。
...
复跑：`node probe-orca.mjs 3`。结果：[第1轮](runs/orca-1.json)、[第2轮](runs/orca-2.json)。
...
复跑：`node probe-paperclip.mjs 3`。结果：[第1轮](runs/paperclip-1.json)、[第2轮](runs/paperclip-2.json)。
